...
class FuelCycleOptimizer:
    ...
    def multi_objective_optimization(
        ...
        def objective(x):
            ...
            cycle_length = np.clip(
                cycle_length, self.min_cycle_length, self.max_cycle_length
            )
        ...
        if method == "genetic_algorithm" and _ADVANCED_OPT_AVAILABLE:
            ...
            logger.info(
                "Using Particle Swarm Optimization for multi-objective optimization"
            )
    ...
    def optimize_batch_configuration(
        ...
        for n_batches in range(2, self.max_batches + 1):
            ...
            for pattern in shuffle_patterns:
                keff, burnup, cost = burnup_solver(n_batches, equal_fractions, pattern)
