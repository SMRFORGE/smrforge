...
def _extract_uncertainty_for_view(
    ...
    *,
    ...
    if unc.shape != dat.shape:
        raise ValueError(
            "mesh_tally.uncertainty must have the same shape as mesh_tally.data"
        )
...
class MeshTally:
    ...
    def get_energy_spectrum(self, position: Tuple[int, ...]) -> np.ndarray:
        ...
        return np.asarray([self.data[position]], dtype=float).reshape(
            -1
        )  # Single group
...
def plot_mesh_tally(
    ...
    **kwargs,
    ...
    """
    if backend == "plotly":
        return _plot_mesh_tally_plotly(
            mesh_tally, geometry, field, energy_group, show_uncertainty, **kwargs
        )
    elif backend == "pyvista":
        return _plot_mesh_tally_pyvista(
            mesh_tally, geometry, field, energy_group, show_uncertainty, **kwargs
        )
    elif backend == "matplotlib":
        return _plot_mesh_tally_matplotlib(
            mesh_tally, geometry, field, energy_group, show_uncertainty, **kwargs
        )
    else:
        ...
        if show_uncertainty and mesh_tally.uncertainty is not None:
            ...
            unc_title = (
                ...
                if str(uncertainty_mode).lower() in ("percent", "pct")
                    ...
                    "Relative uncertainty"
                    if str(uncertainty_mode).lower() in ("relative", "rel")
                    else "Uncertainty"
                )
            )
            ...
            fig.update_layout(
                title=kwargs.pop("suptitle", f"{mesh_tally.name} (value + uncertainty)")
            )
        else:
            fig = go.Figure(
                data=go.Heatmap(
...
def _plot_mesh_tally_pyvista(
    ...
    if mesh_tally.geometry_type == "cylindrical":
        ...
        grid[field] = np.repeat(data[:, :, np.newaxis], len(theta), axis=2).flatten(
            order="F"
        )
...
def _plot_mesh_tally_matplotlib(
    mesh_tally, geometry, field, energy_group, show_uncertainty, **kwargs
):
    ...
    if mesh_tally.geometry_type == "cylindrical":
        ...
        if show_uncertainty and mesh_tally.uncertainty is not None:
            ...
            fig, (ax0, ax1) = plt.subplots(
                1, 2, figsize=kwargs.get("figsize", (14, 6)), sharey=True
            )
...
def plot_multi_group_mesh_tally(
    ...
    **kwargs,
    ...
    if backend == "plotly":
        ...
        for group in range(n_groups):
            fig = plot_mesh_tally(
                mesh_tally, geometry, energy_group=group, backend=backend, **kwargs
            )
            figs.append(fig)
        return figs
