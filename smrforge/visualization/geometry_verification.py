...
def _plot_overlaps_plotly(geometry, overlaps, **kwargs):
    ...
    for comp1, comp2, overlap_region in overlaps:
        ...
        if hasattr(overlap_region, "vertices"):
            ...
            z = [v[2] for v in vertices] if len(vertices[0]) > 2 else [0] * len(vertices)
...
def _plot_overlaps_matplotlib(geometry, overlaps, **kwargs):
    ...
    for comp1, comp2, overlap_region in overlaps:
        if hasattr(overlap_region, "vertices"):
            vertices = overlap_region.vertices()
            poly = Polygon(vertices, alpha=0.7, facecolor="red", edgecolor="darkred", linewidth=2)
            ax.add_patch(poly)
...
def plot_geometry_consistency(
    ...
    **kwargs,
    ...
    if backend == "plotly":
        ...
        return _plot_consistency_matplotlib(geometry, consistency_checks, issues, **kwargs)
    else:
        raise ValueError(f"Unknown backend: {backend}")
...
def _plot_consistency_matplotlib(geometry, consistency_checks, issues, **kwargs):
    ...
    y_pos = 0.95
    for issue in issues:
        ax.text(0.02, y_pos, issue, transform=ax.transAxes,
                bbox=dict(boxstyle="round", facecolor="red" if "error" in issue.lower() else "yellow"),
                fontsize=8)
        y_pos -= 0.05
    ...
    ax.text(0.98, 0.98, check_text, transform=ax.transAxes,
            bbox=dict(boxstyle="round", facecolor="lightgreen" if all(consistency_checks.values()) else "lightyellow"),
