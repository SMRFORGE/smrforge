...
def register_results_callbacks(app):
    ...
    def update_results_summary(results, _nav_clicks):
        ...
        if not results or (isinstance(results, dict) and len(results) == 0):
            return dbc.Alert(
                "No results available. Run an analysis first.", color="info"
            )
        ...
        if "neutronics" in results:
            ...
            if "warning" in neutronics_data:
                card_content.append(
                    html.P(
                        neutronics_data["warning"], className="text-warning small mb-0"
                    )
                )
        ...
        if "burnup" in results:
            ...
            summary_items.append(
                dbc.Row(
                    [dbc.Col([dbc.Alert(message, color="success")])], className="mt-2"
                )
            )
    ...
    def update_3d_plot(results, reactor_spec, _nav_clicks):
        ...
        try:
            ...
            fig = plot_ray_traced_geometry(
                ...
                width=(
                    float(spec.core_diameter) * 1.2,
                    float(spec.core_diameter) * 1.2,
                    float(spec.core_height) * 1.2,
                ),
                basis="xy",
                backend="plotly",
            )
    ...
    def update_transient_plot(results, _nav_clicks):
        ...
        if not results:
            return dbc.Alert(
                "No transient data available. Run an analysis first.", color="info"
            )
        ...
        if (
            ...
            and "safety" in results
            and results["safety"].get("status") == "success"
        ):
            series = results["safety"]
            title = "Safety transient (point kinetics)"
        elif (
            isinstance(results, dict)
            and "quick_transient" in results
            and results["quick_transient"].get("status") == "success"
        ):
            series = results["quick_transient"]
            title = "Quick transient"
        elif (
            isinstance(results, dict)
            and "lumped_thermal" in results
            and results["lumped_thermal"].get("status") == "success"
        ):
        ...
        if not series:
            return dbc.Alert(
                "No transient series found. Run a safety / quick transient / lumped thermal analysis.",
                color="info",
            )
        ...
        if t and series.get("power"):
            fig.add_trace(
                go.Scatter(
                    x=t, y=series.get("power", []), mode="lines", name="Power (W)"
                )
            )
        ...
        if t and (series.get("T_moderator") or series.get("T_mod")):
            fig.add_trace(
                go.Scatter(
                    x=t,
                    y=series.get("T_moderator", series.get("T_mod", [])),
                    mode="lines",
                    name="T_moderator (K)",
                    yaxis="y2",
                )
        ...
        if t and series.get("reactivity"):
            fig.add_trace(
                go.Scatter(
                    x=t,
                    y=series.get("reactivity", []),
                    mode="lines",
                    name="Reactivity (dk/k)",
                    yaxis="y3",
                )
            )
    ...
    def export_results(json_clicks, csv_clicks, plots_clicks, results, reactor_spec):
        ...
        if not results or (isinstance(results, dict) and len(results) == 0):
            return dbc.Alert(
                "No results to export. Run an analysis first.", color="warning"
            )
                ...
                if rows:
                    with open(filepath, "w", newline="") as f:
                        writer = csv.DictWriter(
                            f, fieldnames=["Category", "Parameter", "Value"]
                        )
                        writer.writeheader()
