...
class NuclearDataCache:
    ...
    **ENDF File Setup Required:**
    ...
    def __init__(
        ...
        self._tsl_file_index: Optional[Dict[str, Path]] = (
            None  # TSL material name -> file path
        )
        self._photon_file_index: Optional[Dict[str, Path]] = (
            None  # Element symbol -> file path
        )
        ...
        self._memory_cache: "OrderedDict[str, Tuple[np.ndarray, np.ndarray]]" = (
            OrderedDict()
        )
        ...
        try:
            ...
            if config and "endf" in config:
                ...
                if "default_directory" in endf_config:
                    dir_path = (
                        Path(endf_config["default_directory"]).expanduser().resolve()
                    )
                    if dir_path.exists():
                        return dir_path
    ...
    def _get_file_metadata(self, file_path: Path) -> Tuple[float, int, Optional[set]]:
        ...
        try:
            ...
            if file_path in self._file_metadata_cache:
                cached_mtime, cached_size, cached_mts = self._file_metadata_cache[
                    file_path
                ]
    ...
    def get_parser_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if parser is not None:
                ...
                info["is_cpp_parser"] = (
                    "Cpp" in info["parser_type"]
                    or "cpp" in info["parser_type"].lower()
                    or self._parser_type == "C++"
                )
        except ImportError:  # pragma: no cover
            pass
        ...
        try:
            ...
            if i < len(lines) and len(lines[i]) > 75:
                try:
                    ...
                    if (
                        mf_check == "3"
                        and mt_check == str(reaction_mt)
                        and first_val_str
                        and second_val_str
                        and abs(float(first_val_str)) < 1e-10
                        and abs(float(second_val_str)) < 1e-10
                    ):
            ...
            if len(values) == 0:
                logger.debug(
                    f"No values parsed from ENDF MF=3 section for MT={reaction_mt}"
                )
                return None, None

            if len(values) % 2 != 0:
                logger.warning(
                    f"Odd number of values in ENDF MF=3 section, MT={reaction_mt}, discarding last value"
                )
            ...
            if len(energy_list) > 0 and len(xs_list) > 0:
                ...
                if not np.all(valid_mask):
                    ...
                    if n_removed > len(energy) * 0.5:  # More than 50% invalid
                        logger.warning(
                            f"Found {n_removed} invalid entries in ENDF data for MT={reaction_mt} out of {len(energy)} total"
                        )
                    elif n_removed > 0:
                        logger.debug(
                            f"Filtered {n_removed} invalid entries from ENDF data for MT={reaction_mt}"
                        )
                ...
                if np.any(xs < 0):
                    ...
                    if n_negative > len(xs) * 0.1:  # More than 10% negative
                        logger.warning(
                            f"Found {n_negative} negative cross sections in ENDF data for MT={reaction_mt}, setting to zero"
                        )
                ...
                if len(energy) > 0 and len(xs) > 0 and len(energy) == len(xs):
                    ...
                    logger.debug(
                        f"No valid data remaining after filtering for MT={reaction_mt} (had {len(energy_list)} original points)"
                    )

        except Exception as e:  # pragma: no cover
            logger.debug(
                f"Error parsing ENDF file {endf_file} for MT={reaction_mt}: {e}"
            )

        return None, None

    @staticmethod
    def _extract_mf3_data(
        mf3_mt_data,
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """
        ...
        try:
            ...
            if isinstance(mf3_mt_data, dict):
                ...
                if "data" in mf3_mt_data:
                    ...
                    if isinstance(data, (list, np.ndarray)) and len(data) > 0:
                        ...
                        if (
                            len(data_array.shape) == 1
                            and len(data_array) % 2 == 0
                            and len(data_array) >= 2
                        ):
                ...
                for key, value in mf3_mt_data.items():
                    ...
                    if isinstance(key, str):
                        ...
                        if (
                            ("E" in key_upper or "ENERGY" in key_upper)
                            and "XS" not in key_upper
                            and "CROSS" not in key_upper
                        ):
                        ...
                        # Look for cross-section-like keys (XS, SIGMA, CROSS)
                            ...
                            "XS" in key_upper
                            or "SIGMA" in key_upper
                            or "CROSS" in key_upper
                        ):
                            if hasattr(value, "__iter__") and not isinstance(
                                value, str
                            ):
                ...
                if energy_value is not None and xs_value is not None:
                    ...
                    energy_array = (
                        ...
                        if not isinstance(energy_value, np.ndarray)
                        else energy_value
                    )
                    xs_array = (
                        np.array(xs_value)
                        if not isinstance(xs_value, np.ndarray)
                        else xs_value
                    )
            ...
            if isinstance(mf3_mt_data, (list, tuple)):
                ...
                if mf3_mt_data.dtype.names is not None:
                    ...
                    if (
                        "E" in mf3_mt_data.dtype.names
                        and "XS" in mf3_mt_data.dtype.names
                    ):
                    ...
                    # Try other common field names
                        ...
                        "energy" in mf3_mt_data.dtype.names
                        and "cross_section" in mf3_mt_data.dtype.names
                    ):
            ...
            logger.debug(
                f"Failed to extract MF3 data from endf-parserpy structure: {e}"
            )
    ...
    def _ensure_endf_file(self, nuclide: Nuclide, library: Library) -> Path:
        ...
        if not filepath.exists():
            ...
            if local_file is None:
                ...
                if fallback_library:
                    logger.debug(
                        f"Trying fallback library {fallback_library.value} for {nuclide.name}"
                    )
            ...
            if local_file and local_file.exists():
                ...
                error_msg = (
                    f"ENDF file not found for {nuclide.name} ({library.value}).\n\n"
                )
                error_msg += (
                ...
                if self.local_endf_dir:
                    ...
                    error_msg += (
                        "3. Set local_endf_dir when creating NuclearDataCache:\n"
                    )
    ...
    def _build_local_file_index(self) -> Dict[str, Path]:
        ...
        for endf_file in standard_files:
            ...
            if nuclide:
                logger.debug(
                    f"Found {nuclide.name} at {endf_file.relative_to(self.local_endf_dir)}"
                )
        ...
        for endf_file in alternative_files:
            if endf_file in scanned_files:
                continue
            scanned_files.add(endf_file)
            nuclide = self._add_file_to_index(
                endf_file, index, allow_alternative_names=True
            )
            if nuclide:
                logger.debug(
                    f"Found {nuclide.name} with alternative naming: {endf_file.name}"
                )

        self._local_file_index = index
        logger.info(
            f"Built local ENDF file index: {len(index)} files found in {self.local_endf_dir}"
        )
        return index
    ...
    def _add_file_to_index(
        ...
        endf_file: Path,
        index: Dict[str, Path],
        allow_alternative_names: bool = False,
    ) -> Optional[Nuclide]:
        """
            ...
            if nuclide:
                ...
                if endf_file.name.startswith("n-"):
                    ...
                    if expected_filename != endf_file.name:
                        logger.debug(
                            f"Filename mismatch: expected {expected_filename}, got {endf_file.name}"
                        )
                ...
                if self._validate_endf_file(endf_file):
                    ...
                    if nuclide.name not in index:
                        ...
                        logger.debug(
                            f"Skipping duplicate {nuclide.name}: {endf_file.name} (already have {index[nuclide.name].name})"
                        )
    ...
    def _validate_endf_file(filepath: Path) -> bool:
        ...
        try:
            ...
            has_endf_marker = (
                header[:4] == b"  -1"
                or b"ENDF" in header[:100]
                or b"ENDF/B" in header[:100]
            )
    ...
    def _find_local_endf_file(
        self, nuclide: Nuclide, library: Library
    ) -> Optional[Path]:
        """
        ...
        if file_path is None and library == Library.ENDF_B_VIII_1:
            logger.debug(
                f"VIII.1 file not found for {nuclide.name}, trying VIII.0 fallback"
            )
    ...
    def _find_local_tsl_file(
        self, material_name: str, library: Library = Library.ENDF_B_VIII_1
    ) -> Optional[Path]:
        """
    ...
    def _build_tsl_file_index(self) -> Dict[str, Path]:
        ...
        for tsl_file in unique_files:
            ...
            if material_name_lower not in index:
                ...
                logger.debug(
                    f"Found TSL material '{material_name}' at {tsl_file.relative_to(self.local_endf_dir)}"
                )
            else:
                logger.debug(
                    f"Skipping duplicate TSL material '{material_name}': {tsl_file.name}"
                )
    ...
    def _find_local_fission_yield_file(
        self, nuclide: Nuclide, library: Library
    ) -> Optional[Path]:
        """
    ...
    def _find_local_photon_file(
        self, element: str, library: Library = Library.ENDF_B_VIII_1
    ) -> Optional[Path]:
        """
    ...
    def _build_photon_file_index(self) -> Dict[str, Path]:
        ...
        for photon_file in unique_files:
            ...
            if element_lower not in index:
                ...
                logger.debug(
                    f"Found photon file for '{element}' (Z={Z}) at {photon_file.relative_to(self.local_endf_dir)}"
                )
            else:
                logger.debug(
                    f"Skipping duplicate photon file for '{element}': {photon_file.name}"
                )

        self._photon_file_index = index
        logger.info(
            f"Built photon file index: {len(index)} elements found in {photoat_dir}"
        )
        return index
    ...
    def get_photon_cross_section(
        self, element: str, energy: Optional[float] = None
    ...
    def _build_gamma_production_file_index(self) -> Dict[str, Path]:
        ...
        for gamma_file in unique_files:
            ...
            if nuclide_name not in index:
                ...
                logger.debug(
                    f"Found gamma production file for '{nuclide_name}' at {gamma_file.relative_to(self.local_endf_dir)}"
                )
            else:
                logger.debug(
                    f"Skipping duplicate gamma production file for '{nuclide_name}': {gamma_file.name}"
                )

        logger.info(
            f"Built gamma production file index: {len(index)} nuclides found in {gammas_dir}"
        )
        return index

    def _find_local_gamma_production_file(
        self, nuclide: Nuclide, library: Library = Library.ENDF_B_VIII_1
    ) -> Optional[Path]:
        """
    ...
    def get_gamma_production_data(
        self, nuclide: Nuclide
    ) -> Optional["GammaProductionData"]:
        """
        ...
        """
        gamma_file = self._find_local_gamma_production_file(
...
class CrossSectionTable:
        ...
        for nuclide in nuclides:
            for reaction in reactions:
                ...
                if len(energy) == 0 or len(xs) == 0:
                    skipped_reactions.append(f"{nuclide.name}/{reaction}")
                    logger.warning(f"Skipping {nuclide.name}/{reaction}: Empty data")
                    continue
                if len(energy) != len(xs):
                    raise ValueError(
    ...
    def pivot_for_solver(self, nuclides: List[str], reactions: List[str]) -> np.ndarray:
        ...
        filtered = (
            self.data.lazy()  # Enable lazy evaluation for filtering optimization
            .filter(
...
class DecayData:
    ...
    def _get_half_life(self, nuclide: Nuclide) -> float:
        ...
        try:
            ...
            decay_file = self._cache._find_local_decay_file(
                nuclide, Library.ENDF_B_VIII_1
            )
...
def get_fission_yield_data(
    ...
    **Data Access Pattern:**
    ...
    try:
        ...
        _get_active_logger().warning(
            f"Could not load fission yield data for {nuclide.name}: {e}"
        )
        return None
...
def get_cross_section_with_self_shielding(
    ...
    try:
        ...
        if key in bondarenko.f_factors:
            f_factor = bondarenko.f_factors[key](np.log10(sigma_0), temperature)[
                0, 0
        ...
        logger.warning(
            f"Self-shielding calculation failed: {e}, using infinite dilution"
        )
...
def organize_bulk_endf_downloads(
    ...
    for endf_file in source_dir.rglob("*.endf"):
        ...
        if nuclide.name in index:
            log.debug(
                f"Duplicate {nuclide.name}: {endf_file.name} (keeping {index[nuclide.name].name})"
            )
            stats["files_skipped"] += 1
            continue
        ...
        if create_structure:
            ...
            try:
                ...
                log.debug(
                    f"Organized {nuclide.name}: {endf_file.name} -> {target_file.name}"
                )
...
class NuclideInventoryTracker:
    ...
    def update_nuclide(self, nuclide: Nuclide, atom_density: float) -> None:
        ...
        if nuclide not in self.nuclides:
            raise ValueError(
                f"Nuclide {nuclide.name} not in inventory. Use add_nuclide() first."
            )
        self.atom_densities[nuclide] = atom_density
    ...
    def get_mass_fraction(self, nuclide: Nuclide) -> float:
        ...
        total_mass = sum(self.get_atom_density(nuc) * nuc.A for nuc in self.nuclides)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        return {
            nuclide.name: self.get_atom_density(nuclide) for nuclide in self.nuclides
        }

    def from_dict(
        self, data: Dict[str, float], nuclide_parser: Optional[Any] = None
    ) -> None:
        """
    ...
    def _parse_nuclide_name(
        self, name: str, parser: Optional[Any] = None
    ) -> Optional[Nuclide]:
        """
        ...
        try:
            ...
            logger.debug("Nuclide parsing failed for %r: %s", name, e)
