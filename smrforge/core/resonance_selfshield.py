...
class BondarenkoMethod:
    ...
    def _load_f_factors(self) -> None:
        ...
        f_table_u238_capture = np.array(
            [
                # sigma_0 = 1, 10, 100, 1000, 10000, 1e5, 1e6, 1e10
                [0.95, 0.92, 0.88, 0.84, 0.81, 0.78, 0.76, 0.74],  # T=300K
                [0.93, 0.89, 0.85, 0.80, 0.76, 0.73, 0.70, 0.68],  # T=600K
                [0.91, 0.87, 0.82, 0.76, 0.72, 0.68, 0.65, 0.63],  # T=900K
                [0.89, 0.84, 0.78, 0.72, 0.67, 0.63, 0.60, 0.58],  # T=1200K
                [0.87, 0.81, 0.75, 0.68, 0.63, 0.58, 0.55, 0.53],  # T=1500K
                [0.85, 0.79, 0.72, 0.65, 0.59, 0.54, 0.51, 0.49],  # T=1800K
                [0.83, 0.76, 0.69, 0.62, 0.56, 0.51, 0.48, 0.46],  # T=2100K
            ]
        )

        # Create interpolator (fast 2D spline)
        # RectBivariateSpline(x, y, z) expects:
        # - x: 1D array of length m
        # - y: 1D array of length n
        # - z: 2D array of shape (m, n) where z[i, j] = f(x[i], y[j])
        # Our table is organized as [temperature][sigma_0] = (7, 8)
        # But we need z[i, j] = f(sigma_0[j], T[i])
        # So we need z to be (8, 7) for x=sigma_0 (8 values) and y=T (7 values)
        # Therefore we transpose: (7, 8).T = (8, 7)
        self.f_factors["U238_capture"] = RectBivariateSpline(
            np.log10(sigma_0_grid),
            T_grid,
            f_table_u238_capture.T,  # Transpose to (8, 7) for RectBivariateSpline
            kx=3,
            ky=3,  # Cubic splines
        )
    ...
    def _add_u235_factors(self) -> None:
        ...
        f_table_u235_fission = np.array(
            [
                ...
                [0.96, 0.95, 0.93, 0.91, 0.89, 0.87, 0.86, 0.85],
                [0.94, 0.92, 0.90, 0.87, 0.85, 0.82, 0.80, 0.78],
                [0.92, 0.89, 0.86, 0.83, 0.80, 0.77, 0.75, 0.73],
                [0.90, 0.87, 0.83, 0.79, 0.76, 0.72, 0.70, 0.68],
                [0.88, 0.84, 0.80, 0.76, 0.72, 0.68, 0.65, 0.63],
                [0.86, 0.82, 0.77, 0.72, 0.68, 0.64, 0.61, 0.59],
            ]
        )
    ...
    def _add_pu239_factors(self) -> None:
        ...
        f_table_pu239_fission = np.array(
            [
                ...
                [0.95, 0.93, 0.91, 0.89, 0.87, 0.85, 0.84, 0.83],
                [0.93, 0.90, 0.87, 0.84, 0.81, 0.79, 0.77, 0.75],
                [0.91, 0.87, 0.83, 0.79, 0.76, 0.73, 0.71, 0.69],
                [0.89, 0.84, 0.80, 0.75, 0.71, 0.68, 0.65, 0.63],
                [0.87, 0.82, 0.76, 0.71, 0.67, 0.63, 0.60, 0.58],
                [0.85, 0.79, 0.73, 0.68, 0.63, 0.59, 0.56, 0.54],
            ]
        )

        self.f_factors["Pu239_fission"] = RectBivariateSpline(
            np.log10(sigma_0_grid), T_grid, f_table_pu239_fission.T, kx=3, ky=3
        )

    @lru_cache(maxsize=1024)
    def get_f_factor(
        self, nuclide: str, reaction: str, sigma_0: float, T: float
    ) -> float:
        """
    ...
    def shield_cross_section(
        self, xs_inf: float, nuclide: str, reaction: str, sigma_0: float, T: float
    ) -> float:
        """
...
class SubgroupMethod:
    ...
    def _add_fast_subgroups(self) -> None:
        ...
        self.subgroup_data["U238_fission_fast"] = {"sigma": sigma_sg, "weights": w_sg}

    def compute_effective_xs(
        self, nuclide: str, reaction: str, energy_group: str, sigma_0: float
    ) -> float:
        """
    ...
    def tabulate_effective_xs(
        self, nuclide: str, reaction: str, energy_group: str, sigma_0_range: np.ndarray
    ) -> np.ndarray:
...
class EquivalenceTheory:
    ...
    @staticmethod
    def effective_background_xs(
        dancoff: float, escape_prob: float, sigma_moderator: float, N_fuel_to_mod: float
    ) -> float:
        """
    ...
    def compute_triso_shielding(
        ...
        sigma_0_kernel = self.effective_background_xs(C, P_esc, sigma_graphite, N_ratio)
...
class ResonanceSelfShielding:
    ...
    def htgr_fuel_shielding(
        ...
        for nuclide, N in fuel_composition.items():
            ...
            shielded_xs[nuclide] = {"capture": xs_capture, "fission": xs_fission}
...
if __name__ == "__main__":
    ...
    fuel_comp = {"U235": 0.0005, "U238": 0.0020, "O16": 0.0050}  # atoms/b-cm
