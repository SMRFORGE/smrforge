...
def setup_existing_files() -> Optional[Path]:
    ...
    while True:
        path_input = input("\nEnter path to your ENDF directory (or 'q' to quit): ").strip()
    ...
    try:
        ...
        print(f"  Library versions: {', '.join(results['library_versions']) if results['library_versions'] else 'None detected'}")
        ...
        if results['valid_files'] == 0:
            ...
            retry = input("\nWould you like to try a different directory? (y/n): ").strip().lower()
            if retry == 'y':
                return setup_existing_files()
            return None
    ...
    if organize == 'y':
        ...
        if results['library_versions']:
            ...
            version_input = input(f"Enter library version (default: {library_version}): ").strip()
            if version_input:
    ...
    try:
        ...
        from .reactor_core import Nuclide, Library
        test_nuclides = [
...
def validate_endf_setup(endf_dir: Optional[Path] = None) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        try:
            ...
            found = 0
            for nuclide in test_nuclides:
                if cache._find_local_endf_file(nuclide, Library.ENDF_B_VIII_1) or \
                   cache._find_local_endf_file(nuclide, Library.ENDF_B_VIII):
                    found += 1
            
            if found == 0:
                results["warnings"].append("No test nuclides found (may still be valid)")
