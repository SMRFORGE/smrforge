...
class EnergyAngleData:
    ...
    def _tabular_to_legendre(
        ...
        if moments[0] > 0:
            moments = moments / moments[0] * 0.5  # Normalize so P0 = 0.5 (standard normalization)
...
class ENDFEnergyAngleParser:
    ...
    def parse_file(
        ...
        if nuclide is None:
            ...
            if nuclide is None:
                logger.warning(f"Could not extract nuclide from filename: {filepath.name}")
                return None
    ...
    def _parse_angular_distribution(
        ...
        if law == 1:  # Continuum energy-angle distribution
            ...
            return self._parse_tabular_distribution(lines, start_idx, e_incident, n_points)
            ...
            return self._parse_legendre_distribution(lines, start_idx, e_incident, n_points)
        else:
