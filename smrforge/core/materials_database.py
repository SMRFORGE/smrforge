...
class HeliumCoolant:
    ...
    def reynolds_number(self, T: float, P: float, velocity: float, 
...
class TRISOFuel:
    ...
    def failure_probability(self, T_max: float, burnup: float, 
                           fluence: float) -> float:
        """
        ...
        f_flu = (fluence / fluence_threshold) ** 1.5 if fluence > fluence_threshold else 0.0
...
class MaterialDatabase:
    ...
    def _build_index(self):
        ...
        for name, mat in self.materials.items():
            ...
            records.append({
                'name': name,
                'type': mat_type,
                'category': self._categorize(mat_type)
            })
    ...
    def get(self, name: str):
        ...
        if name not in self.materials:
            raise KeyError(f"Material '{name}' not found. Available: {list(self.materials.keys())}")
    ...
    def compare_properties(self, material_names: list, property_name: str,
                          T_range: np.ndarray) -> pl.DataFrame:
...
@njit(cache=True)
def helium_properties_fast(T: np.ndarray, P: float = 7.0e6) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
