...
class ENDFPhotonParser:
    ...
    def parse_file(self, filepath: Path) -> Optional[PhotonCrossSection]:
        ...
        try:
            ...
            sigma_pe_interp = (
                self._interpolate_to_grid(energy, sigma_pe)
                if sigma_pe
                else np.zeros_like(energy)
            )
            sigma_comp_interp = (
                self._interpolate_to_grid(energy, sigma_comp)
                if sigma_comp
                else np.zeros_like(energy)
            )
            sigma_pair_interp = (
                self._interpolate_to_grid(energy, sigma_pair)
                if sigma_pair
                else np.zeros_like(energy)
            )
    ...
    def _parse_mt_section(
        self, lines: List[str], mt: int
    ...
    def _interpolate_to_grid(
        ...
        # Interpolate
        xs_interp = np.interp(
            target_energy, source_energy, source_xs, left=0.0, right=0.0
        )
