...
class WorthProfile:
    ...
    def get_worth_fraction(self, position: float) -> float:
        ...
        if position > 1.0:
            ...
            position = (
                position / np.max(self.positions) if np.max(self.positions) > 0 else 0.0
            )
    ...
    @classmethod
    def parabolic(
        cls, n_points: int = 20, peak_position: float = 0.5
    ) -> "WorthProfile":
        """
...
class ControlRodWorthCalculator:
    ...
    def _calculate_flux_weighted_worth(
        ...
        worth_fractions = np.array(
            [self.worth_profile.get_worth_fraction(pos) for pos in inserted_positions]
        )

        # Flux-weighted average
        if np.sum(inserted_flux) > 0:
            flux_weighted_worth = np.sum(inserted_flux * worth_fractions) / np.sum(
                inserted_flux
            )
        else:
    ...
    def interpolate_worth(
        ...
        if self.worth_interpolation == "cubic":
            ...
            interp_func = interp1d(
                insertions, calculated_worths, kind="cubic", fill_value="extrapolate"
            )
...
def calculate_worth_profile_from_neutronics(
    ...
    worths = np.array(
        [calculate_rod_worth_pcm(k_eff_critical, k_eff) for k_eff in k_eff_values]
    )
