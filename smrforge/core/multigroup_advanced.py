...
class SPHMethod:
    ...
    def calculate_sph_factors(
        ...
        Example:
            ...
            ... )
        """
        from .reactor_core import CrossSectionTable, NuclearDataCache
...
class EquivalenceTheory:
    ...
    def calculate_equivalent_xs(
        ...
        if dancoff_factor is None:
            dancoff_factor = self._calculate_dancoff_factor(fuel_pin_radius, pin_pitch)
...
def collapse_with_adjoint_weighting(
    ...
    for g_fine in range(n_fine):
        ...
        if is_descending:
            ...
            for c in range(n_coarse):
                if (
                    coarse_group_structure[c]
                    <= E_center
                    < coarse_group_structure[c + 1]
                ):
                    g_coarse = c
                    break
            # Handle edge cases
            ...
            if coarse_flux[g_coarse] > 0:
                ...
                for g_fine in range(n_fine):
                    ...
                    if g_coarse_check == g_coarse:
                        dE = E_high - E_low
                        numerator += (
                            fine_cross_sections[g_fine] * fine_flux[g_fine] * dE
                        )
