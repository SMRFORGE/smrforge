...
class SMRRefuelingPattern:
    ...
    def __post_init__(self):
        ...
        if abs(sum(self.batch_fractions) - 1.0) > 0.01:
            raise ValueError(
                f"Batch fractions must sum to 1.0, got {sum(self.batch_fractions)}"
            )
...
class SMRFuelManager(AssemblyManager):
    ...
    def refuel_smr(
        ...
        self._refuel_rng = np.random.default_rng(seed)
    ...
    def _shuffle_scatter(self, pattern: SMRRefuelingPattern):
        ...
        rng = getattr(self, "_refuel_rng", None) or np.random.default_rng(None)
        ...
        for assembly in self.assemblies:
            if assembly.batch >= pattern.n_batches:
                ...
                # Fresh fuel randomly assigned to batch 1 or 2 (seeded for reproducibility)
                assembly.batch = int(rng.choice([1, 2]))
