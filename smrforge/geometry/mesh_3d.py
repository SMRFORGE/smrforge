...
class Mesh3D:
    ...
    cell_data: Dict[str, np.ndarray] = field(default_factory=dict)  # {field_name: [n_cells]}
    face_data: Dict[str, np.ndarray] = field(default_factory=dict)  # {field_name: [n_faces]}
    ...
    def add_cell_data(self, name: str, data: np.ndarray):
        ...
        if len(data) != self.n_cells:
            raise ValueError(f"Data length {len(data)} does not match cell count {self.n_cells}")
    ...
    def add_face_data(self, name: str, data: np.ndarray):
        ...
        if len(data) != self.n_faces:
            raise ValueError(f"Data length {len(data)} does not match face count {self.n_faces}")
...
class Surface:
    ...
    def to_mesh3d(self) -> Mesh3D:
        ...
        return Mesh3D(
            ...
            faces=self.faces,
            cell_materials=np.array([self.material_id] * self.n_faces) if self.material_id else None,
        )
...
def combine_meshes(meshes: List[Mesh3D]) -> Mesh3D:
    ...
    # Combine cell data
    combined_cell_data = {
        name: np.concatenate(data_list)
        for name, data_list in all_cell_data.items()
    }
