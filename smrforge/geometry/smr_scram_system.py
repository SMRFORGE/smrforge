...
class SMRScramSystem(ControlRodSystem):
    ...
    def scram_smr(
        ...
        if sequence.scram_type == ScramType.EMERGENCY:
            ...
            if sequence.bank_priorities:
                ...
                sorted_banks = sorted(
                    self.banks,
                    key=lambda b: b.priority.value if hasattr(b.priority, 'value') else 0
                )
    ...
    def calculate_scram_time(
        ...
        if rod_length is None:
            ...
            if self.banks:
                rod_lengths = [
                    rod.length for bank in self.banks for rod in bank.rods
                ]
                if rod_lengths:
    ...
    def check_auto_scram(
        ...
        for trigger_name, threshold in self.scram_triggers.items():
            if trigger_name == "temperature" and temperature and temperature > threshold:
                trigger_reason = f"High temperature: {temperature:.1f} K > {threshold:.1f} K"
            elif trigger_name == "pressure" and pressure and pressure > threshold:
                trigger_reason = f"High pressure: {pressure:.2e} Pa > {threshold:.2e} Pa"
    ...
    def get_scram_statistics(self) -> Dict[str, float]:
        ...
        if self.scram_history:
            ...
            last_scram_time = last_scram.timestamp  # Would use current time in real system
        else:
            last_scram_time = 0.0
