...
class PrismaticCore:
    ...
    def _add_channels_to_block(
        self, block: GraphiteBlock, n_fuel: int = 210, n_coolant: int = 102
    ):
        """
    ...
    def get_block_by_position(
        self, x: float, y: float, z: float
    ) -> Optional[GraphiteBlock]:
...
class PebbleBedCore:
    ...
    def _check_no_overlap(
        self, position: Point3D, radius: float, min_separation: float = 0.1
    ) -> bool:
    ...
    def simulate_recirculation(
        self, n_passes: int = 10, discharge_burnup: float = 90.0
    ):
        """
    ...
    def get_pebble_neighbors(
        self, pebble_id: int, radius: float = 10.0
    ) -> List[Pebble]:
            ...
            records.append(
                {
                    ...
                    "temperature": (
                        pebble.material_region.temperature
                        if pebble.material_region
                        else 0.0
                    ),
                }
            )
...
class GeometryExporter:
    ...
    def to_vtk(core: Union[PrismaticCore, PebbleBedCore], filepath: Path):
        ...
        if isinstance(core, PrismaticCore):
            ...
            mesh["temperature"] = [
                p.material_region.temperature if p.material_region else 0.0
...
if __name__ == "__main__":
    ...
    pebble_bed.build_structured_packing(
        core_height=1100.0, core_diameter=300.0, pebble_radius=3.0  # cm  # cm
    )

    console.print(f"   Pebbles: {len(pebble_bed.pebbles)}")
    console.print(f"   Packing fraction: {pebble_bed.packing_fraction:.3f}")
    console.print(
        f"   Core volume: {np.pi * (pebble_bed.core_diameter/2)**2 * pebble_bed.core_height:.0f} cm³"
    )
