...
class CompactSMRCore(PWRSMRCore):
    ...
    def build_compact_core(
        ...
        if len(self.assemblies) > n_assemblies:
            ...
            assemblies_with_dist.sort(key=lambda x: x[1])  # Sort by distance (closest first)
    ...
    def get_compact_core_metrics(self) -> Dict[str, float]:
        ...
        compactness_ratio = (self.n_assemblies / full_scale_assemblies) * \
                           (full_scale_diameter / self.core_diameter) if self.core_diameter > 0 else 0.0
