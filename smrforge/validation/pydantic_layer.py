...
class ReactorSpecification(BaseModel):
    ...
    name: str = Field(
        min_length=1,
        max_length=100,
        description="Reactor design name"
    )
    reactor_type: ReactorType = Field(
        description="Core configuration type"
    )
    ...
    power_thermal: float = Field(
        gt=0,
        le=1e9,
        description="Thermal power [W]"
    )
    power_electric: Optional[float] = Field(
        default=None,
        gt=0,
        le=5e8,
        description="Electric power output [W]"
    )
    ...
    inlet_temperature: float = Field(
        gt=273.15,
        le=900,
        description="Coolant inlet temperature [K]"
    )
    outlet_temperature: float = Field(
        gt=273.15,
        le=1500,
        description="Coolant outlet temperature [K]"
    )
    max_fuel_temperature: float = Field(
        gt=273.15,
        le=3000,
        description="Design limit for fuel temperature [K]"
    )
    ...
    primary_pressure: float = Field(
        gt=0,
        le=50e6,
        description="Primary system pressure [Pa]"
    )
    
    # Core geometry [cm]
    core_height: float = Field(
        gt=0,
        le=2000,
        description="Active core height [cm]"
    )
    core_diameter: float = Field(
        gt=0,
        le=2000,
        description="Active core diameter [cm]"
    )
    reflector_thickness: float = Field(
        ge=0,
        le=200,
        description="Radial reflector thickness [cm]"
    )
    
    # Fuel specification
    fuel_type: FuelType = Field(
        description="Fuel form"
    )
    enrichment: float = Field(
        ge=0,
        le=1.0,
        description="U-235 enrichment (mass fraction)"
    )
    heavy_metal_loading: float = Field(
        gt=0,
        le=50000,
        description="Total heavy metal mass [kg]"
    )
    ...
    coolant_flow_rate: float = Field(
        gt=0,
        le=1000,
        description="Primary coolant mass flow rate [kg/s]"
    )
    ...
    cycle_length: float = Field(
        gt=0,
        le=10000,
        description="Fuel cycle length [days]"
    )
    capacity_factor: float = Field(
        ge=0,
        le=1.0,
        description="Target capacity factor"
    )
    target_burnup: float = Field(
        gt=0,
        le=300,
        description="Target discharge burnup [MWd/kg]"
    )
    ...
    doppler_coefficient: float = Field(
        lt=0,
        ge=-1e-3,
        description="Doppler temperature coefficient [dk/k/K]"
    )
    shutdown_margin: float = Field(
        gt=0,
        le=0.30,
        description="Shutdown margin [dk/k]"
    )
    ...
    def validate_power_density(self):
        ...
        if power_density < 1e5:
            warnings.warn(
                f"Very low power density: {power_density/1e6:.2f} MW/m³"
            )
    ...
    def validate_negative_feedback(cls, v):
        ...
        if v < -1e-4:
            warnings.warn(
                f"Very strong Doppler coefficient: {v*1e5:.2f} pcm/K"
            )
        return v
...
class GeometryParameters(BaseModel):
    ...
    lattice_pitch: Optional[float] = Field(
        ...
        gt=0,
        le=100,
        description="Block-to-block pitch [cm]"
    )
    block_height: Optional[float] = Field(
        default=None,
        gt=0,
        le=200,
        description="Individual block height [cm]"
    )
    n_axial_blocks: Optional[int] = Field(
        default=None,
        ge=1,
        le=50,
        description="Number of axial block layers"
    )
    flat_to_flat: Optional[float] = Field(
        default=36.0,
        gt=0,
        le=100,
        description="Block flat-to-flat distance [cm]"
    )
    ...
    pebble_diameter: Optional[float] = Field(
        ...
        gt=0,
        le=10,
        description="Fuel pebble diameter [cm]"
    )
    packing_fraction: Optional[float] = Field(
        default=0.61,
        gt=0,
        le=1.0,
        description="Pebble packing fraction"
    )
    ...
    n_radial_mesh: int = Field(
        ...
        ge=5,
        le=200,
        description="Number of radial mesh cells"
    )
    n_axial_mesh: int = Field(
        default=50,
        ge=10,
        le=500,
        description="Number of axial mesh cells"
    )
...
class CrossSectionData(BaseModel):
    ...
    sigma_t: PositiveArray = Field(
        description="Total cross section [1/cm]"
    )
    sigma_a: PositiveArray = Field(
        description="Absorption cross section [1/cm]"
    )
    sigma_f: PositiveArray = Field(
        description="Fission cross section [1/cm]"
    )
    nu_sigma_f: PositiveArray = Field(
        description="Nu*fission cross section [1/cm]"
    )
    
    # Scattering matrix [material, from_group, to_group]
    sigma_s: PositiveArray = Field(
        description="Scattering matrix [1/cm]"
    )
    
    # Fission spectrum [material, group]
    chi: NormalizedArray = Field(
        description="Fission spectrum (must sum to 1)"
    )
    
    # Diffusion coefficients [material, group]
    D: PositiveArray = Field(
        description="Diffusion coefficient [cm]"
    )
    ...
    def validate_physical_relationships(self):
        ...
        if np.any(self.sigma_a > self.sigma_t):
            raise ValueError(
                "Absorption XS cannot exceed total XS (non-physical)"
            )
...
class SolverOptions(BaseModel):
    ...
    inner_solver: Literal["gauss_seidel", "sor", "bicgstab"] = Field(
        ...
        description="Inner iteration solver"
    )
    eigen_method: Literal["power", "arnoldi"] = Field(
        default="power",
        description="Eigenvalue method"
    )
    verbose: bool = Field(
        default=True,
    ...
    omega: float = Field(
        ...
        gt=0,
        le=2.0,
        description="SOR relaxation parameter"
    )
...
class MaterialComposition(BaseModel):
    ...
    temperature: float = Field(
        gt=0,
        le=4000,
        description="Material temperature [K]"
    )
    
    density: float = Field(
        gt=0,
        le=30,
        description="Physical density [g/cm³]"
    )
    ...
    def validate_composition(self):
        ...
        for nuclide, density in self.composition.items():
            if density < 0:
                raise ValueError(
                    f"Negative atom density for {nuclide}: {density}"
                )
            if density > 0.2:
                warnings.warn(
...
class TransientConditions(BaseModel):
    ...
    initial_power: float = Field(
        gt=0,
        description="Initial power level [W]"
    )
    initial_temperature: float = Field(
        gt=273.15,
        le=3000,
        description="Initial average temperature [K]"
    )
    initial_flow_rate: float = Field(
        ge=0,
        description="Initial coolant flow rate [kg/s]"
    )
    ...
    transient_type: str = Field(
        description="Type of transient"
    )
    trigger_time: float = Field(
        ge=0,
        description="Time when transient initiates [s]"
    )
    ...
    scram_available: bool = Field(
        default=True,
        description="Is reactor scram available"
    )
    scram_delay: float = Field(
        default=1.0,
        ge=0,
        le=10,
        description="Scram actuation delay [s]"
    )
...
class SMRForgeSettings(BaseSettings):
    ...
    nuclear_data_library: Literal["endfb8.0", "jeff3.3", "jendl5.0"] = Field(
        default="endfb8.0",
        description="Default nuclear data library"
    )
