...
class StandardsParser:
    ...
    def parse_ansi_ans_5_1(
        self, data_file: Union[Path, str], encoding: str = "utf-8"
        ...
        try:
            ...
            logger.info(
                f"Parsed {len(benchmarks)} ANSI/ANS-5.1 benchmarks from {data_file}"
            )
            return benchmarks
    ...
    def parse_iaea_benchmark(
        ...
        try:
            with open(data_file, "r", encoding=encoding) as f:
                if (
                    data_file.suffix.lower() == ".yaml"
                    or data_file.suffix.lower() == ".yml"
                ):
                    try:
                        import yaml
            ...
            test_case = (
                data.get("test_case") or data.get("problem_name") or data_file.stem
            )
            ...
            try:
                if benchmark.standard_type == StandardType.ANSI_ANS_5_1:
                    ...
                    burnup_bm = BurnupBenchmark(
                        ...
                        benchmark_compositions=bm_data.get(
                    ...
                    if "benchmark_flux" in bm_data or "benchmark_dose_rate" in bm_data:
                        ...
                        benchmark_dose_rate = BenchmarkValue(
                            value=bm_data.get("benchmark_dose_rate", {}).get(
                                "value", 0.0
                            ),
                            uncertainty=bm_data.get("benchmark_dose_rate", {}).get(
                                "uncertainty"
                            ),
                            unit=bm_data.get("benchmark_dose_rate", {}).get(
                                "unit", "Sv/h"
                            ),
                            source=f"MCNP {bm_data.get('mcnp_version', '')}",
                            notes="",
                        ...
                        gamma_bm = GammaTransportBenchmark(
                            test_case=bm_data.get("test_case", benchmark.test_case),
                            geometry_description=bm_data.get(
                ...
                logger.warning(
                    f"Failed to load benchmark {benchmark.test_case} into database: {e}"
                )
                continue
