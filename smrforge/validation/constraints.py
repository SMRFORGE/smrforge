...
class ConstraintSet:
    ...
    def get_regulatory_limits(cls) -> "ConstraintSet":
        ...
        constraints = cls(
            name="regulatory_limits", description="Standard regulatory safety limits"
        )

        # Typical PWR limits (adjust as needed)
        constraints.add_constraint(
            "max_power_density", 100.0, "max", "W/cm³", "Maximum local power density"
        )
        constraints.add_constraint(
            "max_temperature", 1200.0, "max", "K", "Maximum fuel temperature"
        )
        constraints.add_constraint(
            "min_k_eff", 1.0, "min", "", "Minimum criticality (safety margin)"
        )
        constraints.add_constraint(
            "max_burnup", 50.0, "max", "MWd/kg", "Maximum fuel burnup"
        )
    ...
    def get_safety_margins(cls) -> "ConstraintSet":
        ...
        constraints = cls(
            name="safety_margins", description="Safety margin requirements"
        )

        constraints.add_constraint(
            "shutdown_margin", 0.005, "min", "", "Shutdown margin (k_eff < 0.995)"
        )
        constraints.add_constraint(
            "power_peak_factor", 1.5, "max", "", "Power peaking factor"
        )
...
class DesignValidator:
    ...
    def validate(
        self, reactor, analysis_results: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        """
        ...
        if analysis_results is None:
            try:
                ...
                return ValidationResult(
                    passed=False, violations=violations, metrics=metrics
                )
        ...
        if "power" in analysis_results:
            ...
            if isinstance(power_array, np.ndarray):
                ...
                metrics["power_peak_factor"] = float(
        ...
        for constraint_name, constraint_def in self.constraint_set.constraints.items():
            ...
            if constraint_type == "max":
                ...
                if violation:
                    ...
                    severity = (
                        "warning" if limit * 0.9 < value <= limit * 1.1 else "error"
                    )
                else:
        ...
        return ValidationResult(
            passed=passed, violations=violations, warnings=warnings, metrics=metrics
        )
