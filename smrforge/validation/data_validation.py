...
class ValidationResult:
    ...
    def add_issue(self, level: ValidationLevel, parameter: str, 
                  message: str, **kwargs):
    ...
    def has_errors(self) -> bool:
        ...
        return any(i.level in [ValidationLevel.ERROR, ValidationLevel.CRITICAL] 
    ...
    def print_report(self, console: Optional[Console] = None):
        ...
        # Detailed issues
        if any(summary[level.value] > 0 for level in 
...
class PhysicalValidator:
    ...
    @staticmethod
    def validate_temperature(T: float, name: str = "temperature",
                           min_T: float = 273.0,
                           max_T: float = 3000.0) -> ValidationResult:
        """Validate temperature value."""
        result = ValidationResult(valid=True)
        
        if not isinstance(T, (int, float)):
            result.add_issue(ValidationLevel.ERROR, name,
                           "Must be numeric", value=type(T))
            return result
        
        if np.isnan(T) or np.isinf(T):
            result.add_issue(ValidationLevel.ERROR, name,
                           "Invalid value (NaN or Inf)", value=T)
            return result
        
        if T <= PhysicalValidator.ABSOLUTE_ZERO:
            result.add_issue(ValidationLevel.ERROR, name,
                           "Below absolute zero", value=T,
                           expected=f"> {PhysicalValidator.ABSOLUTE_ZERO} K")
        
        if T < min_T:
            result.add_issue(ValidationLevel.WARNING, name,
                           "Below expected minimum", value=T,
                           expected=f">= {min_T} K")
        
        if T > max_T:
            result.add_issue(ValidationLevel.WARNING, name,
                           "Above expected maximum", value=T,
                           expected=f"<= {max_T} K")
        
        if T > PhysicalValidator.MAX_TEMPERATURE:
            result.add_issue(ValidationLevel.ERROR, name,
                           "Exceeds physical limits", value=T,
                           expected=f"<= {PhysicalValidator.MAX_TEMPERATURE} K")
        
        return result
    
    @staticmethod
    def validate_pressure(P: float, name: str = "pressure",
                         min_P: float = 1e5,
                         max_P: float = 10e6) -> ValidationResult:
        """Validate pressure value."""
        result = ValidationResult(valid=True)
        
        if not isinstance(P, (int, float)):
            result.add_issue(ValidationLevel.ERROR, name,
                           "Must be numeric", value=type(P))
            return result
        
        if np.isnan(P) or np.isinf(P):
            result.add_issue(ValidationLevel.ERROR, name,
                           "Invalid value", value=P)
            return result
        
        if P < PhysicalValidator.MIN_PRESSURE:
            result.add_issue(ValidationLevel.ERROR, name,
                           "Negative pressure", value=P)
        
        if P < min_P:
            result.add_issue(ValidationLevel.WARNING, name,
                           "Below expected minimum", value=P,
                           expected=f">= {min_P} Pa")
        
        if P > max_P:
            result.add_issue(ValidationLevel.WARNING, name,
                           "Above expected maximum", value=P,
                           expected=f"<= {max_P} Pa")
        
        return result
    
    @staticmethod
    def validate_enrichment(enrichment: float, 
                          fuel_type: str = "LEU") -> ValidationResult:
        """Validate fuel enrichment."""
        result = ValidationResult(valid=True)
        
        if not isinstance(enrichment, (int, float)):
            result.add_issue(ValidationLevel.ERROR, "enrichment",
                           "Must be numeric", value=type(enrichment))
            return result
        
        if enrichment < PhysicalValidator.MIN_ENRICHMENT:
            result.add_issue(ValidationLevel.ERROR, "enrichment",
                           "Below zero", value=enrichment)
        
        if enrichment > PhysicalValidator.MAX_ENRICHMENT:
            result.add_issue(ValidationLevel.ERROR, "enrichment",
                           "Above 100%", value=enrichment)
        
        # Check fuel type limits
        if fuel_type == "LEU" and enrichment > 0.20:
            result.add_issue(ValidationLevel.ERROR, "enrichment",
                           "Exceeds LEU limit (20%)", value=enrichment,
                           expected="<= 0.20")
        
        elif fuel_type == "HALEU" and enrichment > 0.20:
            if enrichment > 0.20 and enrichment <= 0.05:
                result.add_issue(ValidationLevel.WARNING, "enrichment",
                               "In HALEU range (>20%, <5%)", value=enrichment)
        
        elif fuel_type == "HEU" and enrichment < 0.20:
            result.add_issue(ValidationLevel.WARNING, "enrichment",
                           "Below HEU threshold", value=enrichment)
        
        return result
    
    @staticmethod
    def validate_power(power: float, reactor_type: str = "SMR",
                      min_power: float = 1e3,
                      max_power: float = 1e9) -> ValidationResult:
        """Validate power level."""
        result = ValidationResult(valid=True)
        
        if not isinstance(power, (int, float)):
            result.add_issue(ValidationLevel.ERROR, "power",
                           "Must be numeric", value=type(power))
            return result
        
        if power <= 0:
            result.add_issue(ValidationLevel.ERROR, "power",
                           "Must be positive", value=power)
        
        if power < min_power:
            result.add_issue(ValidationLevel.WARNING, "power",
                           "Very low power", value=power,
                           expected=f">= {min_power} W")
        
        if power > max_power:
            result.add_issue(ValidationLevel.WARNING, "power",
                           "Very high power", value=power,
                           expected=f"<= {max_power} W")
        
        # Type-specific checks
        if reactor_type == "micro" and power > 10e6:
            result.add_issue(ValidationLevel.WARNING, "power",
                           "High for micro-reactor", value=power,
                           expected="<= 10 MWth")
        
        elif reactor_type == "SMR" and power > 300e6:
            result.add_issue(ValidationLevel.WARNING, "power",
                           "Exceeds typical SMR range", value=power,
                           expected="<= 300 MWth")
    ...
    def validate_k_eff(k_eff: float, margin: float = 0.02) -> ValidationResult:
        ...
        if not isinstance(k_eff, (int, float)):
            result.add_issue(ValidationLevel.ERROR, "k_eff",
                           "Must be numeric", value=type(k_eff))
            return result
        
        if k_eff <= 0:
            result.add_issue(ValidationLevel.ERROR, "k_eff",
                           "Must be positive", value=k_eff)
        
        if k_eff < 0.9:
            result.add_issue(ValidationLevel.ERROR, "k_eff",
                           "Too subcritical", value=k_eff,
                           expected=">= 0.9")
        
        if k_eff > 1.2:
            result.add_issue(ValidationLevel.ERROR, "k_eff",
                           "Too supercritical", value=k_eff,
                           expected="<= 1.2")
        
        # Operating range check
        if k_eff < 1.0 - margin:
            result.add_issue(ValidationLevel.WARNING, "k_eff",
                           "Below critical with margin", value=k_eff,
                           expected=f">= {1.0 - margin}")
        
        if k_eff > 1.0 + margin:
            result.add_issue(ValidationLevel.INFO, "k_eff",
                           "Above critical with margin", value=k_eff)
...
class GeometryValidator:
    ...
    @staticmethod
    def validate_dimensions(height: float, diameter: float,
                          min_size: float = 1.0,
                          max_size: float = 2000.0) -> ValidationResult:
        """Validate core dimensions."""
        result = ValidationResult(valid=True)
        
        # Height validation
        if height <= 0:
            result.add_issue(ValidationLevel.ERROR, "core_height",
                           "Must be positive", value=height)
        
        if height < min_size:
            result.add_issue(ValidationLevel.WARNING, "core_height",
                           "Very small", value=height,
                           expected=f">= {min_size} cm")
        
        if height > max_size:
            result.add_issue(ValidationLevel.WARNING, "core_height",
                           "Very large", value=height,
                           expected=f"<= {max_size} cm")
        
        # Diameter validation
        if diameter <= 0:
            result.add_issue(ValidationLevel.ERROR, "core_diameter",
                           "Must be positive", value=diameter)
        
        if diameter < min_size:
            result.add_issue(ValidationLevel.WARNING, "core_diameter",
                           "Very small", value=diameter,
                           expected=f">= {min_size} cm")
        
        if diameter > max_size:
            result.add_issue(ValidationLevel.WARNING, "core_diameter",
                           "Very large", value=diameter,
                           expected=f"<= {max_size} cm")
        
        # Aspect ratio check
        aspect_ratio = height / diameter
        if aspect_ratio < 0.5:
            result.add_issue(ValidationLevel.WARNING, "aspect_ratio",
                           "Very flat core (H/D < 0.5)", value=aspect_ratio)
        
        if aspect_ratio > 5.0:
            result.add_issue(ValidationLevel.WARNING, "aspect_ratio",
                           "Very tall core (H/D > 5)", value=aspect_ratio)
    ...
    def validate_mesh(n_radial: int, n_axial: int) -> ValidationResult:
        ...
        if n_radial < 5:
            result.add_issue(ValidationLevel.WARNING, "n_radial",
                           "Coarse radial mesh", value=n_radial,
                           expected=">= 10")
        
        if n_radial > 200:
            result.add_issue(ValidationLevel.WARNING, "n_radial",
                           "Very fine radial mesh (slow)", value=n_radial,
                           expected="<= 100")
        
        if n_axial < 10:
            result.add_issue(ValidationLevel.WARNING, "n_axial",
                           "Coarse axial mesh", value=n_axial,
                           expected=">= 20")
        
        if n_axial > 500:
            result.add_issue(ValidationLevel.WARNING, "n_axial",
                           "Very fine axial mesh (slow)", value=n_axial,
                           expected="<= 200")
...
class NeutronicsValidator:
    ...
    def validate_cross_sections(xs_data) -> ValidationResult:
        ...
        if np.any(xs_data.sigma_t < 0):
            result.add_issue(ValidationLevel.ERROR, "sigma_t",
                           "Negative total cross section")
        
        if np.any(xs_data.sigma_a < 0):
            result.add_issue(ValidationLevel.ERROR, "sigma_a",
                           "Negative absorption cross section")
        
        if np.any(xs_data.sigma_f < 0):
            result.add_issue(ValidationLevel.ERROR, "sigma_f",
                           "Negative fission cross section")
        
        # Check physical bounds
        if np.any(xs_data.sigma_a > xs_data.sigma_t):
            result.add_issue(ValidationLevel.ERROR, "cross_sections",
        ...
        if np.any(xs_data.D <= 0):
            result.add_issue(ValidationLevel.ERROR, "D",
                           "Non-positive diffusion coefficient")
        
        if np.any(xs_data.D > 10):
            result.add_issue(ValidationLevel.WARNING, "D",
        ...
        if not np.allclose(chi_sum[chi_sum > 0], 1.0, rtol=1e-3):
            result.add_issue(ValidationLevel.WARNING, "chi",
    ...
    def validate_flux(flux: np.ndarray, power: float) -> ValidationResult:
        ...
        if np.any(flux < 0):
            result.add_issue(ValidationLevel.ERROR, "flux",
                           "Negative flux values")
        
        if np.any(np.isnan(flux)):
            result.add_issue(ValidationLevel.CRITICAL, "flux",
                           "NaN in flux solution")
        
        if np.any(np.isinf(flux)):
            result.add_issue(ValidationLevel.CRITICAL, "flux",
                           "Inf in flux solution")
        
        # Check reasonableness
        max_flux = np.max(flux)
        if max_flux < 1e10:
            result.add_issue(ValidationLevel.WARNING, "flux",
                           "Very low flux", value=max_flux,
                           expected=">= 1e12 n/cm²-s")
        
        if max_flux > 1e16:
            result.add_issue(ValidationLevel.WARNING, "flux",
                           "Very high flux", value=max_flux,
...
class ThermalValidator:
    ...
    def validate_heat_transfer(h: float, regime: str = "turbulent") -> ValidationResult:
        ...
        if h <= 0:
            result.add_issue(ValidationLevel.ERROR, "h",
                           "Non-positive heat transfer coefficient", value=h)
        
        if regime == "laminar" and h > 1000:
            result.add_issue(ValidationLevel.WARNING, "h",
                           "Too high for laminar flow", value=h,
                           expected="< 1000 W/m²-K")
        
        if regime == "turbulent" and h < 100:
            result.add_issue(ValidationLevel.WARNING, "h",
                           "Too low for turbulent flow", value=h,
                           expected="> 100 W/m²-K")
    ...
    def validate_reynolds_number(Re: float) -> ValidationResult:
        ...
        if Re < 0:
            result.add_issue(ValidationLevel.ERROR, "Re",
                           "Negative Reynolds number", value=Re)
        
        if Re < 1:
            result.add_issue(ValidationLevel.WARNING, "Re",
                           "Very low Re (creeping flow)", value=Re)
        
        if Re > 1e7:
            result.add_issue(ValidationLevel.WARNING, "Re",
                           "Very high Re", value=Re,
                           expected="< 1e7")
        
        # Flow regime info
        if 2300 < Re < 4000:
            result.add_issue(ValidationLevel.INFO, "Re",
                           "Transitional flow regime", value=Re)
...
class ConsistencyValidator:
    ...
    @staticmethod
    def validate_energy_balance(power_generated: float,
                               power_removed: float,
        ...
        if error > tolerance:
            result.add_issue(ValidationLevel.ERROR, "energy_balance",
                           f"Mismatch > {tolerance*100}%",
                           value=f"{error*100:.2f}%")
        elif error > tolerance / 2:
            result.add_issue(ValidationLevel.WARNING, "energy_balance",
                           "Small imbalance", value=f"{error*100:.2f}%")
        
        return result
    
    @staticmethod
    def validate_material_conservation(mass_in: float, mass_out: float,
        ...
        if relative_error > tolerance:
            result.add_issue(ValidationLevel.ERROR, "mass_balance",
                           "Conservation violated", value=f"{relative_error:.2e}")
...
class DataValidator:
    ...
    def validate_reactor_spec(self, spec) -> ValidationResult:
        ...
        if spec.inlet_temperature >= spec.outlet_temperature:
            result.add_issue(ValidationLevel.ERROR, "temperatures",
                           "Inlet >= Outlet (non-physical)")
        ...
        enr_result = self.physical.validate_enrichment(
            spec.enrichment, spec.fuel_type
        )
        result.issues.extend(enr_result.issues)
        
        power_result = self.physical.validate_power(
            spec.power_thermal, spec.reactor_type.value if hasattr(spec, 'reactor_type') else "SMR"
        )
    ...
    def validate_solver_inputs(self, geometry, xs_data, options) -> ValidationResult:
        ...
        mesh_result = self.geometry.validate_mesh(
            len(geometry.radial_mesh) - 1,
            len(geometry.axial_mesh) - 1
        )
        ...
        if options.tolerance < 1e-10:
            result.add_issue(ValidationLevel.WARNING, "tolerance",
                           "Very tight tolerance (slow)", value=options.tolerance)
        
        if options.tolerance > 1e-3:
            result.add_issue(ValidationLevel.WARNING, "tolerance",
                           "Loose tolerance (inaccurate)", value=options.tolerance)
        
        if options.max_iterations < 10:
            result.add_issue(ValidationLevel.WARNING, "max_iterations",
                           "May not converge", value=options.max_iterations)
        
        result.valid = not result.has_errors()
        
        return result
    
    def validate_solution(self, k_eff: float, flux: np.ndarray,
                         power: np.ndarray, power_target: float) -> ValidationResult:
        ...
        if power_error > 0.01:
            result.add_issue(ValidationLevel.WARNING, "power_normalization",
                           "Power normalization error", value=f"{power_error*100:.2f}%")
