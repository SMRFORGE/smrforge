...
class CapitalCostEstimator:
    ...
    direct_costs: Dict[str, float] = field(default_factory=lambda: {
    ...
    indirect_costs: Dict[str, float] = field(default_factory=lambda: {
    ...
    def __post_init__(self):
        ...
        if self.base_cost_per_kwe is None:
            ...
            self.base_cost_per_kwe = base_costs.get(
                self.reactor_type.lower(), 6000.0
            )
    ...
    def get_cost_breakdown(self) -> Dict[str, float]:
        ...
        for component, fraction in self.direct_costs.items():
            breakdown[f"direct_{component}"] = cost_before_contingency * (fraction / (direct_total + indirect_total))
        
        # Indirect costs
        for component, fraction in self.indirect_costs.items():
            breakdown[f"indirect_{component}"] = cost_before_contingency * (fraction / (direct_total + indirect_total))
...
class LCOECalculator:
    ...
    def calculate_lcoe(self) -> float:
        ...
        annual_generation_kwh = (
            power_kw * hours_per_year * self.capacity_factor
        )
    ...
    def get_cost_breakdown(self) -> Dict[str, float]:
        ...
        pv_capital = self.capital_cost
        capital_contribution = pv_capital / (
