...
class FluidProperties:
    ...
    def viscosity(self) -> float:
        ...
        mu_ref = 1.865e-5
        C = 79.4
        return mu_ref * (self.temperature / T_ref) ** 1.5 * (T_ref + C) / (self.temperature + C)
...
class ChannelThermalHydraulics:
    ...
    def solve_steady_state(self, T_fuel: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        """
        ...
        for i in range(self.nz):
            ...
            dP_friction = -f * (self.dz / 100) / (self.geom.hydraulic_diameter / 100) * \
                         0.5 * rho * v**2
            
            # Acceleration pressure drop (usually small)
            rho_next = self.P_coolant[i] / (2077.0 * self.T_coolant[i+1])
            dP_accel = self.mdot**2 * (1/(rho_next * self.geom.flow_area * 1e-4)**2 - 
    ...
    def _compute_wall_temperature(self, T_fuel: float, T_coolant: float,
                                  h: float, q_linear: float) -> float:
        """
...
class FuelRodThermal:
    ...
    def solve_steady_conduction(self, q_vol: float, k_fuel: float,
                               T_surface: float) -> np.ndarray:
        """
    ...
    def solve_transient_conduction(self, q_vol: np.ndarray, k_fuel: float,
                                   rho: float, cp: float,
                                   T_surface: float, t_span: Tuple[float, float],
                                   T_initial: Optional[np.ndarray] = None) -> Dict:
        """
        ...
        return {
            't': sol.t,
            'T': sol.y  # Shape: (n_nodes+1, n_times)
        }
...
class PorousMediaFlow:
    ...
    def __init__(self, bed_height: float, bed_diameter: float,
                 pebble_diameter: float, porosity: float):
    ...
    def solve_flow(self, mdot: float, T_in: float, P_in: float,
                   q_vol_profile: np.ndarray) -> Dict:
        """
...
@njit(cache=True)
def solve_tridiagonal_fast(a: np.ndarray, b: np.ndarray, c: np.ndarray,
...
class ConjugateHeatTransfer:
    ...
    def solve_coupled(self, max_iterations: int = 50, 
...
if __name__ == "__main__":
    ...
    console.print(f"   Outlet temperature: {result['T_coolant'][-1]:.1f} K "
                 f"({result['T_coolant'][-1]-273:.1f}°C)")
    console.print(f"   Outlet pressure: {result['P_coolant'][-1]/1e6:.2f} MPa")
    console.print(f"   Pressure drop: {(inlet['pressure'] - result['P_coolant'][-1])/1e3:.1f} kPa")
    ...
    console.print(f"   Pressure drop: {(bed_result['P_coolant'][0] - bed_result['P_coolant'][-1])/1e3:.1f} kPa")
