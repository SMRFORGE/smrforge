        ...
        for iteration in range(self.options.max_iterations):
            ...
            if hasattr(self.neutronics_solver, "compute_power_distribution"):
                if initial_power is None:
                    ...
                    power = initial_power
                power_distribution = self.neutronics_solver.compute_power_distribution(
                    power
                )
            else:
            ...
            if self.options.include_control and self.control_system is not None:
                if iteration % self.options.control_update_frequency == 0:
                    if hasattr(self.control_system, "compute_reactivity_adjustment"):
                        reactivity_adj = (
                            self.control_system.compute_reactivity_adjustment(
            ...
            if hasattr(self.thermal_solver, "solve_with_power"):
                temperature_new = self.thermal_solver.solve_with_power(
                    power_distribution
                )
            else:
            ...
            if (
                self.options.include_structural
                and self.structural_mechanics is not None
            ):
            ...
            if error < self.options.tolerance:
                logger.info(
                    f"Multi-physics coupling converged in {iteration+1} iterations"
                )
                self.converged = True
