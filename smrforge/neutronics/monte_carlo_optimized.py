            ...
            if d_collision < d_boundary:
                ...
                if xi < sigma_s:
                    ...
                    if n_fissions < max_fissions - 2:
                        ...
                        nu = 2.5
                        n_fiss = int(nu) + (
                            1 if np.random.random() < (nu - int(nu)) else 0
                        )
...
class OptimizedMonteCarloSolver:
    ...
    def __init__(
        ...
        # Use local RNG (avoids global np.random.seed for reproducibility)
    ...
    def _build_xs_tables(self):
        ...
        for mat in range(n_materials):
            for g in range(n_groups):
                ...
                self.sigma_s_table[mat, g] = self.xs_data.sigma_s[
                    mat, g, 0
                ]  # Simplified
        ...
        logger.debug(f"Built XS tables: {n_materials} materials, {n_groups} groups")
    ...
    def run_eigenvalue(self) -> Dict:
        ...
        """
        # Set legacy RNG state for Numba JIT functions (they use np.random)
    ...
    def _initialize_source(self):
        ...
        for _ in range(self.n_particles):
            # Uniform in cylinder (use instance RNG for reproducibility)
            r = self.geometry.r_core * np.sqrt(self._rng.random())
            theta = 2 * np.pi * self._rng.random()
            x = r * np.cos(theta)
            y = r * np.sin(theta)
            z = self.geometry.h_core * self._rng.random()
    ...
    def _resample_source(self):
        ...
        # Randomly select with replacement (use instance RNG for reproducibility)
        self.source_bank.clear()
