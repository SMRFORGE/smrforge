...
class GadoliniumDepletion:
    ...
    def get_capture_cross_section(
        self, nuclide: Nuclide, temperature: float = 600.0
    ) -> float:
        """
    ...
    def calculate_reactivity_worth(
        ...
        for i, nuclide in enumerate(gd_poison.nuclides):
            ...
            depletion_fraction = (
                1.0 - (final_conc / initial_conc) if initial_conc > 0 else 0.0
            )
...
class AssemblyWiseBurnupTracker:
    ...
    def __init__(
        self, n_assemblies: int, lattice_size: Optional[Tuple[int, int]] = None
    ):
        """
        ...
        logger.debug(
            f"AssemblyWiseBurnupTracker initialized: {n_assemblies} assemblies"
        )
    ...
    def get_assembly_position(self, assembly_id: int) -> Tuple[int, int]:
        ...
        if assembly_id < 0 or assembly_id >= self.n_assemblies:
            raise ValueError(
                f"assembly_id must be 0-{self.n_assemblies-1}, got {assembly_id}"
            )
