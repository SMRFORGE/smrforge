...
class GammaTransportSolver:
    ...
    def __init__(
        ...
        if geometry.axial_mesh is not None and geometry.radial_mesh is not None:
            self.r_centers = 0.5 * (
                geometry.radial_mesh[1:] + geometry.radial_mesh[:-1]
            )
    ...
    def _initialize_cross_sections(self, material: str = "H2O"):
        ...
        if comp and len(comp.elements) > 1:
            ...
            if cross_sections:
                ...
                logger.info(
                    f"Loaded weighted photon cross-sections for {material} from ENDF data"
                )
            else:
    ...
    def _load_photon_data_for_element(
        ...
        try:
            ...
            density = (
                MaterialMapper.get_density("H2O") or 1.0
    ...
    def solve(
        self,
        source: Union[np.ndarray, Tuple[np.ndarray, np.ndarray]],
        ...
        for iteration in range(self.options.max_iterations):
            ...
            # Check convergence
            if iteration > 0:
                error = np.max(np.abs(self.flux - flux_old)) / (
                    np.max(np.abs(self.flux)) + 1e-10
                )

                if self.options.verbose and iteration % 10 == 0:
                    logger.debug(
                        f"Gamma transport iteration {iteration}: error = {error:.2e}"
                    )

                if error < self.options.tolerance:
                    logger.info(
                        f"Gamma transport converged in {iteration+1} iterations"
                    )
                    return self.flux
    ...
    def _build_group_system(
        self, g: int, source_g: np.ndarray
        ...
        radial_contrib[:, 1:-1] = (
            2 * radial_diffusion_coef[1:-1, np.newaxis] * cell_volumes[:, 1:-1]
        )  # Inner cells
        radial_contrib[:, 0] = (
            radial_diffusion_coef[0] * cell_volumes[:, 0]
        )  # Left boundary
        radial_contrib[:, -1] = (
            radial_diffusion_coef[-1] * cell_volumes[:, -1]
        )  # Right boundary

        # Add axial diffusion contributions (vectorized)
        axial_contrib = np.zeros((self.nz, self.nr))
        axial_contrib[1:-1, :] = (
            2 * axial_diffusion_coef[1:-1, np.newaxis] * cell_volumes[1:-1, :]
        )  # Inner cells
        axial_contrib[0, :] = (
            axial_diffusion_coef[0] * cell_volumes[0, :]
        )  # Bottom boundary
        axial_contrib[-1, :] = (
            axial_diffusion_coef[-1] * cell_volumes[-1, :]
        )  # Top boundary
