"""
Validation command handlers: validate_run, report_design, validate_benchmark, validate_design.
"""
...
def validate_run(args):
    ...
    try:
        ...
        try:
            # commands module is one level deeper than cli.py; need 3 parents to reach smrforge pkg, 4 for project root
            script_path = Path(__file__).parent.parent.parent.parent / "scripts" / "run_validation.py"
        ...
        result = subprocess.run(
            cmd, cwd=Path(__file__).parent.parent.parent.parent, capture_output=not args.verbose
        )
