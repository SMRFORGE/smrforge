...
Note: All features are also available via Python API:
  ...
  k_eff = reactor.solve_keff()
    ...
    # workflow surrogate-validate (Pro: validation report)
    surval_parser = workflow_subparsers.add_parser(
        "surrogate-validate",
        help="Generate surrogate validation report (Pro). Compare surrogate to physics on holdout set.",
    )
    surval_parser.add_argument("--surrogate", type=Path, required=True, help="Path to surrogate model (.pkl, .pt, .onnx)")
    surval_parser.add_argument("--holdout", type=Path, required=True, help="Path to holdout results JSON")
    surval_parser.add_argument("--params", nargs="+", required=True, help="Parameter names")
    surval_parser.add_argument("--metric", type=str, default="k_eff")
    surval_parser.add_argument("--output", type=Path, help="Output report path (writes .json and .md)")
    surval_parser.add_argument("--format", choices=["json", "md", "both"], default="both")
    surval_parser.set_defaults(func=workflow_surrogate_validate)

    # workflow code-verify (Pro: code-to-code verification)
    cv_parser = workflow_subparsers.add_parser(
        "code-verify",
        help="Code-to-code verification (Pro). Run diffusion, OpenMC, Serpent, MCNP on same reactor.",
    )
    cv_parser.add_argument("--reactor", type=str, required=True, help="Reactor file or preset")
    cv_parser.add_argument(
        "--codes",
        nargs="+",
        default=["diffusion", "openmc"],
        help="Codes to run: diffusion, mc, openmc, serpent, mcnp",
    )
    cv_parser.add_argument("--output", type=Path, help="Save report JSON")
    cv_parser.set_defaults(func=workflow_code_verify)

    # workflow regulatory-package (Pro)
    rp_parser = workflow_subparsers.add_parser(
        "regulatory-package",
        help="Generate regulatory submission package (Pro). NRC/IAEA: inputs, outputs, traceability.",
    )
    rp_parser.add_argument("--reactor", type=str, required=True, help="Reactor file or preset")
    rp_parser.add_argument("--output-dir", type=Path, default=Path("regulatory_package"))
    rp_parser.add_argument("--preset", type=str, default="10_CFR_50")
    rp_parser.set_defaults(func=workflow_regulatory_package)

    # workflow benchmark (Pro)
    bench_parser = workflow_subparsers.add_parser(
        "benchmark",
        help="Reproduce benchmark and compare to reference (Pro).",
    )
    bench_parser.add_argument("--benchmark", type=str, required=True, help="Benchmark ID")
    bench_parser.add_argument("--output-dir", type=Path, help="Output directory")
    bench_parser.set_defaults(func=workflow_benchmark)

    # workflow multi-optimize (Pro)
    mo_parser = workflow_subparsers.add_parser(
        "multi-optimize",
        help="Multi-objective optimization (Pro). k_eff, safety, economics.",
    )
    mo_parser.add_argument("--reactor", type=Path, required=True, help="Base design JSON")
    mo_parser.add_argument(
        "--params",
        nargs="+",
        required=True,
        help="name:low:high for each design variable",
    )
    mo_parser.add_argument(
        "--objectives",
        nargs="+",
        default=["k_eff:max"],
        help="objective:direction e.g. k_eff:max cost:min",
    )
    mo_parser.add_argument("--max-iter", type=int, default=50)
    mo_parser.add_argument("--output", type=Path, help="Save optimization result JSON")
    ...
    bundle_parser.add_argument(
        ...
        type=Path,
        dest="endf_dir",
        help="ENDF directory (default: SMRFORGE_ENDF_DIR or ENDF-B-VIII.1)",
    )
    bundle_parser.add_argument(
        "--output",
        type=Path,
        help="Output archive (default: nuclear-data-bundle.tar.gz)",
    )
