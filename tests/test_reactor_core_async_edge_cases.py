...
class TestAsyncGatherErrorHandling:
        ...
        with patch.object(
            ...
            with pytest.raises((FileNotFoundError, Exception)):
                await table.generate_multigroup_async(
                    nuclides=[u235, u238], reactions=["fission"], group_structure=groups
                )
    ...
    async def test_generate_multigroup_async_all_fail(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_failure
        ):
            with pytest.raises((ImportError, Exception)):
                await table.generate_multigroup_async(
                    nuclides=[u235], reactions=["fission"], group_structure=groups
                )
    ...
    async def test_generate_multigroup_async_empty_inputs(self, mock_cache):
        ...
        result = await table.generate_multigroup_async(
            nuclides=[], reactions=[], group_structure=groups
        )
    ...
    async def test_generate_multigroup_async_missing_data(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_none
        ):
...
class TestAsyncConcurrentAccess:
    ...
    async def test_concurrent_get_cross_section_async(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs
        ):
            # Launch concurrent requests
            tasks = [
                mock_cache.get_cross_section_async(
                    u235, "fission", 900.0, Library.ENDF_B_VIII, None
                ),
                mock_cache.get_cross_section_async(
                    u238, "fission", 900.0, Library.ENDF_B_VIII, None
                ),
            ]
...
class TestAsyncExceptionPropagation:
    ...
    async def test_async_exception_propagates_correctly(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_error
        ):
...
class TestAsyncTimeoutScenarios:
    ...
    async def test_async_slow_operation(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_slow
        ):
    ...
    async def test_async_with_timeout(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_hang
        ):
...
class TestAsyncDataConsistency:
    ...
    async def test_async_results_order_preserved(self, mock_cache):
        ...
        with patch.object(
            mock_cache, "get_cross_section_async", side_effect=mock_get_xs_ordered
        ):
            await table.generate_multigroup_async(
                nuclides=[u235, u238], reactions=["fission"], group_structure=groups
            )
