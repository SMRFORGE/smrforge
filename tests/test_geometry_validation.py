...
class TestValidateGeometryCompleteness:
    ...
    def test_validate_prismatic_core_block_count_warning(self):
        ...
        core.blocks = [
            GraphiteBlock(
                id=i, position=Point3D(0, 0, 0), flat_to_flat=36.0, height=800.0
            )
            for i in range(5)
        ]
...
class TestValidateMaterialConnectivity:
    ...
    def test_validate_connectivity_isolated_block(self, simple_prismatic_core):
        ...
        report = validate_material_connectivity(
            simple_prismatic_core, check_continuity=True
        )

        # Should find isolated block warning
        assert any("isolated" in issue.lower() for issue in report.connectivity_issues)

    def test_validate_material_connectivity_checks_disabled(
        self, simple_prismatic_core
    ):
...
class TestComprehensiveValidation:
    ...
    def test_comprehensive_validation_with_gaps(
        self, simple_prismatic_core, blocks_with_gaps
    ):
...
class TestGeometryValidationExtended:
    ...
    def test_validate_material_connectivity_check_boundaries(
        self, simple_prismatic_core
    ):
    ...
    def test_check_distances_fuel_channel_too_close_to_edge(
        self, simple_prismatic_core
    ):
    ...
    def test_check_distances_coolant_channels_too_close(self, simple_prismatic_core):
        ...
        channel2 = CoolantChannel(
            ...
            position=Point3D(
                13.0, 0, 0
            ),  # Too close (distance 3.0, but radii 3.0 each = 6.0 total, need 2.0 separation = 8.0 min)
            radius=3.0,
            height=800.0,
            flow_area=10.0,
        )
    ...
    def test_validate_assembly_placement_spacing_issue(self, simple_prismatic_core):
        ...
        assert any(
            "spacing" in issue.lower() or "too close" in issue.lower()
            for issue in report.assembly_issues
        )

    def test_validate_assembly_placement_inconsistent_z_levels(
        self, simple_prismatic_core
    ):
    ...
    def test_validate_fuel_loading_pattern_fuel_block_no_channels(
        self, simple_prismatic_core
    ):
    ...
    def test_comprehensive_validation_gap_errors(self, simple_prismatic_core):
        ...
        gap_errors = [
            ...
            if "gap" in err.lower() or "overlap" in err.lower()
        ]
