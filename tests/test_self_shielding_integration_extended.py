...
class TestSelfShieldingIntegrationExtended:
    ...
    def test_get_cross_section_with_self_shielding_enable_false(self):
        ...
        with patch.object(cache, 'get_cross_section', return_value=(mock_energy, mock_xs)) as mock_get:
    ...
    def test_get_cross_section_with_self_shielding_resonance_not_available(self):
        ...
        with patch.object(cache, 'get_cross_section', return_value=(mock_energy, mock_xs)):
            with patch.object(self_shielding_module, '_RESONANCE_AVAILABLE', False):
                with patch.object(self_shielding_module.logger, 'warning') as mock_warning:
                    energy, xs = get_cross_section_with_self_shielding(
                        cache, u238, "capture", temperature=900.0,
                        sigma_0=10.0, method="bondarenko"
                    )
            ...
            with patch.object(self_shielding_module, '_RESONANCE_AVAILABLE', False):
                with patch.object(self_shielding_module.logger, 'warning') as mock_warning:
                    energy, xs = get_cross_section_with_equivalence_theory(
                        ...
                        fuel_pin_radius=0.4, pin_pitch=1.26,
    ...
    def test_get_cross_section_with_self_shielding_subgroup_zero_mean_resonance_xs(self):
        """Test subgroup method when mean(resonance_xs) is zero."""
        cache = NuclearDataCache()
        u238 = Nuclide(Z=92, A=238)
        
        # Create energy grid with resonance region
        mock_energy = np.array([1.0, 10.0, 100.0, 1000.0, 1e5])  # Includes resonance region
