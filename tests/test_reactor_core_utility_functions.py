...
class TestGetParser:
    ...
    def test_get_parser_cpp_parser_available(self, tmp_path):
        ...
        with patch('smrforge.core.reactor_core.logger'):
            with patch('builtins.__import__', side_effect=lambda name, *args, **kwargs: Mock() if name == 'endf_parserpy' else __import__(name, *args, **kwargs)):
                # Mock EndfParserCpp import
                mock_cpp = Mock()
                with patch.dict('sys.modules', {'endf_parserpy': Mock(EndfParserCpp=mock_cpp, EndfParserFactory=Mock())}):
...
class TestGetFissionYieldData:
    ...
    def test_get_fission_yield_data_with_cache(self, mock_endf_file):
        ...
        with patch.object(cache, '_find_local_fission_yield_file', return_value=mock_endf_file):
            # Mock parser
            mock_parser = Mock()
    ...
    @pytest.mark.skip(reason="ImportError testing is complex - exception handling already covered by parser_exception test")
...
class TestOrganizeBulkEndfDownloads:
    ...
    def source_dir(self, tmp_path):
        ...
        content += " 1.000000+5 1.000000+1 1.000000+6 1.200000+1                       125 3  1    3\n" * 50
    ...
    def test_organize_bulk_endf_downloads_default_target(self, source_dir):
        ...
        with patch('smrforge.core.reactor_core.get_standard_endf_directory', return_value=Path.home() / "ENDF-Data"):
            stats = organize_bulk_endf_downloads(
                source_dir=source_dir,
                library_version="VIII.1",
                create_structure=True
...
class TestScanEndfDirectory:
    ...
    def test_scan_endf_directory_multiple_versions(self, tmp_path):
        ...
        (v8_0_dir / "n-092_U_235.endf").write_text(content_v8_0 + " 1.000000+5 1.000000+1\n" * 50)
        (v8_1_dir / "n-092_U_238.endf").write_text(content_v8_1 + " 1.000000+5 1.000000+1\n" * 50)
        
        results = scan_endf_directory(endf_dir)
        
        # Should detect both versions
        assert "VIII.0" in results["library_versions"] or "VIII.1" in results["library_versions"]
