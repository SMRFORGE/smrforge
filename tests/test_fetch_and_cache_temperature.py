...
"""
    ...
    def test_fetch_and_cache_applies_doppler_broadening(
        self, temp_dir, realistic_endf_file_for_temp
    ):
        ...
        with patch.object(
            cache, "_ensure_endf_file", return_value=realistic_endf_file_for_temp
        ):
            ...
            with patch.object(cache, "_doppler_broaden") as mock_doppler:
                ...
                with patch.object(
                    cache, "_simple_endf_parse", return_value=(test_energy, test_xs)
                ):
                    mock_doppler.return_value = broadened_xs

                    # Mock other backends as unavailable
                    with patch.dict(
                        "sys.modules", {"endf_parserpy": None}, clear=False
                    ):
                        with patch.dict("sys.modules", {"sandy": None}, clear=False):
                            with patch(
                                "smrforge.core.reactor_core.ENDFCompatibility",
                                None,
                                create=True,
                            ):
    ...
    def test_fetch_and_cache_no_broadening_at_standard_temp(
        self, temp_dir, realistic_endf_file_for_temp
    ):
    ...
    def test_fetch_and_cache_broadening_tolerance(
        self, temp_dir, realistic_endf_file_for_temp
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                cache, "_simple_endf_parse", return_value=(base_energy, base_xs)
            ):
                with patch.object(cache, "_doppler_broaden") as mock_doppler:
                    # Mock other backends as unavailable
                    with patch.dict(
    ...
    def test_fetch_and_cache_broadening_high_temperature(
        self, temp_dir, realistic_endf_file_for_temp
    ):
                ...
                with patch.object(cache, "_doppler_broaden") as mock_doppler:
                    ...
                    with patch.object(cache, "_get_parser", return_value=None):
                        with patch.dict(
                            ...
                            with patch.dict(
                                "sys.modules", {"sandy": None}, clear=False
                            ):
                                with patch(
                                    "smrforge.core.reactor_core.ENDFCompatibility",
                                    None,
    ...
    def test_fetch_and_cache_broadening_low_temperature(
        self, temp_dir, realistic_endf_file_for_temp
    ):
    ...
    def test_fetch_and_cache_broadening_multiple_temperatures(
        self, temp_dir, realistic_endf_file_for_temp
    ):
