...
class TestENDFFileDiscovery:
    ...
    def test_neutron_file_discovery(self, cache_with_endf):
        ...
        neutron_file = cache_with_endf._find_local_endf_file(
            u235, Library.ENDF_B_VIII_1
        )
    ...
    def test_fission_yield_file_discovery(self, cache_with_endf):
        ...
        u235 = Nuclide(Z=92, A=235)
        yield_file = cache_with_endf._find_local_fission_yield_file(
            u235, Library.ENDF_B_VIII_1
        )
    ...
    def test_gamma_production_file_discovery(self, cache_with_endf):
        ...
        u235 = Nuclide(Z=92, A=235)
        gamma_file = cache_with_endf._find_local_gamma_production_file(
            u235, Library.ENDF_B_VIII_1
        )
...
class TestENDFDataParsing:
    ...
    def test_decay_data_parsing(self, cache_with_endf):
        ...
        try:
            decay_file = cache_with_endf._find_local_decay_file(
                u235, Library.ENDF_B_VIII_1
            )
            decay_data = None
            if decay_file:
            ...
            if decay_data:
                ...
                print(
                    f"[OK] Parsed decay data: lambda = {decay_data.decay_constant:.2e} s^-1"
                )
        except Exception as e:
        ...
        try:
            ...
            if yield_data:
                if len(yield_data.yields) > 0:
                    print(
                        f"[OK] Parsed fission yields: {len(yield_data.yields)} products"
                    )
                else:
    ...
    def test_tsl_parsing(self, cache_with_endf):
        ...
        if len(materials) > 0:
            try:
                tsl_data = get_thermal_scattering_data(
                    materials[0], cache=cache_with_endf
                )
                ...
                if tsl_data:
                    ...
                    print(
                        f"[OK] Parsed TSL data for {materials[0]}: "
        ...
        if len(elements) > 0:
            try:
                ...
                if photon_data:
                    ...
                    print(
                        f"[OK] Parsed photon data for {elements[0]}: {len(photon_data.energy)} points"
                    )
            except Exception as e:
    ...
    def test_gamma_production_parsing(self, cache_with_endf):
        ...
        try:
            ...
            if gamma_data:
                assert (
                    len(gamma_data.prompt_spectra) > 0
                    or len(gamma_data.delayed_spectra) > 0
                )
                print(
                    f"[OK] Parsed gamma production data: "
...
class TestNeutronicsWorkflow:
    ...
    def test_neutronics_with_endf_data(self, cache_with_endf):
        ...
        try:
            ...
            print(
                f"[OK] Neutronics workflow: k_eff = {k_eff:.6f}, "
...
class TestBurnupWorkflow:
    ...
    def test_burnup_with_endf_data(self, cache_with_endf):
        ...
        if decay_data:
            assert decay_data.decay_constant >= 0
            print(
                f"[OK] Burnup workflow: Decay data accessible (lambda = {decay_data.decay_constant:.2e} s^-1)"
            )
...
class TestDecayHeatWorkflow:
    ...
    def test_gamma_source_generation(self, cache_with_endf):
        ...
        try:
            ...
            print(
                f"[OK] Gamma source generation: shape {gamma_source.shape}, "
...
class TestGammaTransportWorkflow:
    ...
    def test_gamma_transport_with_photon_data(self, cache_with_endf):
        ...
        try:
            ...
            print(
                f"[OK] Gamma transport workflow: max flux = {np.max(flux):.4e} photons/cm^2/s, "
    ...
    def test_gamma_transport_time_dependent(self, cache_with_endf):
        ...
        try:
            ...
            print(
                f"[OK] Time-dependent gamma transport: max flux = {np.max(flux):.4e} photons/cm^2/s"
            )
        except Exception as e:
...
class TestIntegratedWorkflow:
    ...
    def test_complete_reactor_analysis(self, cache_with_endf):
        ...
        try:
            ...
            print(
                f"[OK] Step 4: Decay heat calculated "
        ...
        transport = GammaTransportSolver(
            geometry, transport_options, cache=cache_with_endf
        )
        n_axial = len(geometry.axial_mesh) - 1
        n_radial = len(geometry.radial_mesh) - 1
        try:
            ...
            print(
                f"[OK] Step 6: Gamma transport solved "
                f"(max dose = {np.max(dose_rate):.4e} Sv/h)"
            )
        except Exception:
