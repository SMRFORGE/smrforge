...
class TestZarrCacheKeyError:
    ...
    def test_get_cross_section_zarr_keyerror_falls_back_to_fetch(self, temp_dir, mock_endf_file):
        ...
        with patch.object(cache, '_fetch_and_cache', return_value=(
            np.array([1e5, 1e6]), np.array([10.0, 12.0])
        )) as mock_fetch:
            energy, xs = cache.get_cross_section(nuc, "total", 293.6, Library.ENDF_B_VIII)
...
class TestBackendPaths:
    ...
    @pytest.mark.skip(reason="Complex mocking required - backend paths tested in integration tests")
    ...
    @pytest.mark.skip(reason="Complex mocking required - error paths tested in integration tests")
...
class TestSimpleEndfParseControlRecords:
    ...
    def test_simple_endf_parse_skips_second_control_record(self, temp_dir, realistic_endf_file):
...
class TestCollapseToMultigroup:
    ...
    def test_collapse_to_multigroup_custom_weighting(self, temp_dir):
        ...
        result = CrossSectionTable._collapse_to_multigroup(energy, xs, group_bounds, weighting_flux)
