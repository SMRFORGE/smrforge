...
class TestEndfSetupEdgeCases:
    ...
    def test_setup_existing_files_retry_path(self, tmp_path):
        ...
        try:
            ...
            with patch("builtins.input", side_effect=[str(endf_dir), "y", "q"]):
                with patch("builtins.print"):  # Suppress output
                    with patch(
                        "smrforge.core.endf_setup.scan_endf_directory"
                    ) as mock_scan:
            ...
            with patch("builtins.input", side_effect=[str(endf_dir), "", "y", ""]):
                with patch("builtins.print"):  # Suppress output
                    with patch(
                        ...
                        with patch(
                            "smrforge.core.endf_setup.organize_bulk_endf_downloads"
                        ) as mock_org:
                            with patch(
                                "smrforge.core.endf_setup.NuclearDataCache"
                            ) as mock_cache:
                                ...
                                mock_cache_instance._find_local_endf_file.return_value = (
                                    Mock()
                                )
    ...
    def test_setup_download_instructions_yes_path(self):
        ...
        try:
            ...
            with patch(
                "builtins.input", side_effect=["", "y", "q"]
            ):  # 'q' to cancel setup
                with patch("builtins.print"):  # Suppress output
                    with patch(
                        "smrforge.core.endf_setup.setup_existing_files",
                        return_value=None,
                    ):
                        result = setup_download_instructions()
                        assert result is None
