...
from smrforge.core.decay_parser import BetaSpectrum, DecayData, GammaSpectrum
...
class TestDecayHeatCalculatorComprehensive:
    ...
    def test_calculate_decay_heat_with_gamma_spectrum(
        self, mock_cache, mock_decay_data
    ):
    ...
    def test_initial_time_offset(self, mock_cache, mock_decay_data):
        ...
        with patch.object(calculator, "_get_decay_data", return_value=mock_decay_data):
            result = calculator.calculate_decay_heat(
                concentrations, times, initial_time=initial_time
            )
    ...
    def test_get_decay_data_with_file_loading(self, mock_cache):
        ...
        from smrforge.core.decay_parser import DecayData, ENDFDecayParser
        ...
        with patch(
            "smrforge.core.decay_parser.ENDFDecayParser", return_value=mock_parser
        ):
    ...
    def test_get_gamma_energy_per_decay_with_gamma_production(
        self, mock_cache, mock_decay_data
    ):
    ...
    def test_calculate_gamma_source_with_decay_spectrum(
        self, mock_cache, mock_decay_data
    ):
    ...
    def test_calculate_gamma_source_zero_total_energy(
        self, mock_cache, mock_decay_data
    ):
