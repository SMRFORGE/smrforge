...
from smrforge.validation.safety_report import SafetyMarginReport, MarginEntry
...
class TestPlotSensitivityRanking:
    def test_plot_sensitivity_ranking_plotly(self):
        ...
        rankings = [SensitivityRanking("a", 0.8, 1, "oat"), SensitivityRanking("b", 0.3, 2, "oat")]
...
class TestPlotSobolWorkflow:
    def test_plot_sobol_workflow_plotly(self):
        ...
        sobol_dict = {"Y0": {"S1": [0.1, 0.5], "ST": [0.2, 0.6], "param_names": ["a", "b"]}}
...
class TestPlotScenarioComparison:
    def test_plot_scenario_comparison_plotly(self):
        ...
        results = {
            "s1": ScenarioResult("s1", True, {"k_eff": 1.0, "power_thermal_mw": 50}, [], {}),
            "s2": ScenarioResult("s2", False, {"k_eff": 0.98, "power_thermal_mw": 50}, ["v1"], {}),
        }
...
class TestPlotParetoWithKnee:
    def test_plot_pareto_with_knee_plotly(self):
        ...
        fig = plot_pareto_with_knee(x, y, mask, knee_index=0, metric_x="m1", metric_y="m2", backend="plotly")
        assert fig is not None
...
class TestPlotAtlasDesigns:
    def test_plot_atlas_designs_plotly(self):
        ...
        entries = [
            {"design_id": "a", "power_mw": 50, "metrics_summary": {"k_eff": 1.02}, "passed": True},
            {"design_id": "b", "power_mw": 100, "metrics_summary": {"k_eff": 0.99}, "passed": False},
        ]
