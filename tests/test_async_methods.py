...
class TestNuclearDataCacheAsync:
    ...
    def test_get_cross_section_async_memory_cache_hit(
        self, temp_dir, mock_httpx_available
    ):
    ...
    def test_get_cross_section_async_zarr_cache_hit(
        self, temp_dir, mock_httpx_available
    ):
    ...
    def test_get_cross_section_async_cache_miss_fetches(
        self, temp_dir, mock_httpx_available, realistic_endf_file_for_async
    ):
        ...
        with patch.object(
            cache,
            "_fetch_and_cache_async",
            new_callable=AsyncMock,
            return_value=(mock_energy, mock_xs),
        ) as mock_fetch:
    ...
    def test_ensure_endf_file_async_existing_file(
        self, temp_dir, mock_httpx_available, realistic_endf_file_for_async
    ):
        ...
        async def run_test():
            result_file = await cache._ensure_endf_file_async(nuc, Library.ENDF_B_VIII)
            assert result_file == expected_file
            assert result_file.exists()

        asyncio.run(run_test())

    def test_ensure_endf_file_async_downloads_when_missing(
        self, temp_dir, mock_httpx_available, realistic_endf_file_for_async
    ):
    ...
    def test_ensure_endf_file_async_uses_provided_client(
        self,
        temp_dir,
        mock_httpx_available,
        mock_async_client,
    ...
    def test_ensure_endf_file_async_httpx_unavailable(
        self, temp_dir, mock_httpx_unavailable
    ):
    ...
    def test_fetch_and_cache_async_simple_parser(
        self, temp_dir, mock_httpx_available, realistic_endf_file_for_async
    ):
    ...
    def test_fetch_and_cache_async_with_client(
        self,
        temp_dir,
        mock_httpx_available,
        mock_async_client,
...
class TestCrossSectionTableAsync:
    ...
    def test_generate_multigroup_async_basic(self, temp_dir, mock_httpx_available):
        ...
        from smrforge.core.reactor_core import CrossSectionTable, Library, Nuclide
    ...
    def test_generate_multigroup_async_parallel_fetching(
        self, temp_dir, mock_httpx_available
    ):
    ...
    def test_generate_multigroup_async_with_client(
        self, temp_dir, mock_httpx_available, mock_async_client
    ):
    ...
    def test_generate_multigroup_async_creates_client(
        self, temp_dir, mock_httpx_available
    ):
    ...
    def test_generate_multigroup_async_httpx_unavailable(
        self, temp_dir, mock_httpx_unavailable
    ):
    ...
    def test_generate_multigroup_async_with_weighting_flux(
        self, temp_dir, mock_httpx_available
    ):
