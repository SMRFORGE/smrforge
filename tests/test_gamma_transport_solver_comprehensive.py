...
class TestGammaTransportSolverComprehensive:
    ...
    def test_initialize_cross_sections_simple_material(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_initialize_cross_sections_compound_material(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_interpolate_time_dependent_source(self, simple_geometry, mock_cache):
        ...
        source_at_3600 = solver._interpolate_time_dependent_source(
            source_4d, times, 3600.0
        )
        assert source_at_3600.shape == (solver.nz, solver.nr, solver.ng)

        source_at_5400 = solver._interpolate_time_dependent_source(
            source_4d, times, 5400.0
        )  # Interpolate
        assert source_at_5400.shape == (solver.nz, solver.nr, solver.ng)

        source_before = solver._interpolate_time_dependent_source(
            source_4d, times, -1000.0
        )  # Extrapolate
        assert source_before.shape == (solver.nz, solver.nr, solver.ng)

        source_after = solver._interpolate_time_dependent_source(
            source_4d, times, 20000.0
        )  # Extrapolate
    ...
    def test_convergence(self, simple_geometry, mock_cache):
        ...
        options = GammaTransportOptions(
            n_groups=5, max_iterations=100, tolerance=1e-6, verbose=True
        )
        solver = GammaTransportSolver(simple_geometry, options, cache=mock_cache)
    ...
    def test_initialize_cross_sections_compound_material_with_data(
        self, simple_geometry, mock_cache
    ):
        ...
        with patch.object(MaterialMapper, "get_composition", return_value=mock_comp):
            with patch.object(
                MaterialMapper,
                "compute_weighted_cross_section",
                return_value=np.ones(20) * 0.1,
            ):
                solver = GammaTransportSolver(
                    simple_geometry, options, cache=mock_cache
                )
    ...
    def test_initialize_cross_sections_fallback_primary_element(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_solve_convergence_verbose(self, simple_geometry, mock_cache):
        ...
        options = GammaTransportOptions(
            n_groups=5, max_iterations=5, tolerance=1e-3, verbose=True
        )
        solver = GammaTransportSolver(simple_geometry, options, cache=mock_cache)
    ...
    def test_solve_no_convergence(self, simple_geometry, mock_cache):
        ...
        options = GammaTransportOptions(
            n_groups=5, max_iterations=2, tolerance=1e-10, verbose=False
        )
        solver = GammaTransportSolver(simple_geometry, options, cache=mock_cache)
    ...
    def test_interpolate_time_dependent_source_single_point(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_interpolate_time_dependent_source_exact_match(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_interpolate_time_dependent_source_before_start(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_interpolate_time_dependent_source_after_end(
        self, simple_geometry, mock_cache
    ):
    ...
    def test_interpolate_time_dependent_source_validation(
        self, simple_geometry, mock_cache
    ):
