...
class TestParetoReportCoverage:
    ...
    def test_pareto_summary_report_extremes_maximize_false(self):
        ...
        out = pareto_summary_report(points, "cost", "perf", maximize_x=False, maximize_y=False)
...
class TestFuelCycleOptimizationCoverage:
    ...
    def test_fuel_cycle_optimizer_optimize_cycle_length(self):
        ...
        opt = FuelCycleOptimizer(power_thermal=1e9, max_cycle_length=1500.0, min_cycle_length=500.0)
...
class TestConvertersCommunityPath:
    ...
    def test_openmc_export_writes_xml_files(self, tmp_path):
        ...
        OpenMCConverter.export_reactor(mock_reactor, out)
    ...
    def test_converters_pro_delegation_when_mocked(self, tmp_path):
        ...
        with patch.object(conv, "_PRO_AVAILABLE", True), \
             patch.object(conv, "_ProSerpentConverter", mock_serpent), \
...
class TestParameterSweepCoverage:
    ...
    def test_parameter_sweep_run_resume_all_done(self, tmp_path):
        ...
        inter_file.write_text(json.dumps({
            "results": [{"parameters": {"x": 0.0}, "k_eff": 1.0}, {"parameters": {"x": 1.0}, "k_eff": 1.0}],
            "failed": [],
        }))
    ...
    @patch("smrforge.convenience.create_reactor", create=True)
    def test_parameter_sweep_run_sequential_single_case(self, mock_create_reactor, tmp_path):
