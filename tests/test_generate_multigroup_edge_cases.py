...
class TestGenerateMultigroupEdgeCases:
    ...
    def test_generate_multigroup_none_cross_section(
        self, temp_dir, pre_populated_cache
    ):
        ...
        with patch.object(
            pre_populated_cache, "get_cross_section", return_value=(None, None)
        ) as mock_get:
            df = table.generate_multigroup(
                nuclides=[u235],
                reactions=["total"],
    ...
    def test_generate_multigroup_empty_cross_section(
        self, temp_dir, pre_populated_cache
    ):
    ...
    def test_generate_multigroup_mismatched_energy_xs(
        self, temp_dir, pre_populated_cache
    ):
        ...
        with patch.object(pre_populated_cache, "get_cross_section") as mock_get:
            ...
            with patch.object(
                CrossSectionTable, "_collapse_to_multigroup"
            ) as mock_collapse:
    ...
    def test_generate_multigroup_very_large_group_structure(
        self, temp_dir, pre_populated_cache
    ):
    ...
    def test_generate_multigroup_weighting_flux_mismatch(
        self, temp_dir, pre_populated_cache
    ):
    ...
    def test_generate_multigroup_negative_temperature(
        self, temp_dir, pre_populated_cache
    ):
    ...
    def test_generate_multigroup_dataframe_persistence(
        self, temp_dir, pre_populated_cache
    ):
