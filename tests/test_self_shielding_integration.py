...
class TestSelfShieldingIntegration:
    ...
    def test_disable_self_shielding(self):
        ...
        try:
            energy, xs = get_cross_section_with_self_shielding(
                cache, u238, "capture", temperature=900.0, enable_self_shielding=False
            )
    ...
    def test_get_cross_section_with_self_shielding_unknown_method(self):
        ...
        try:
            with pytest.raises(ValueError, match="Unknown self-shielding method"):
                get_cross_section_with_self_shielding(
                    cache, u238, "capture", temperature=900.0, method="unknown_method"
                )
    ...
    def test_get_cross_section_with_self_shielding_cross_section_error(self):
        ...
        with patch.object(
            cache, "get_cross_section", side_effect=FileNotFoundError("Data not found")
        ):
            with pytest.raises(FileNotFoundError):
                get_cross_section_with_self_shielding(
                    cache, u238, "capture", temperature=900.0, method="bondarenko"
                )
    ...
    def test_get_cross_section_with_equivalence_theory_with_moderator_xs(self):
        ...
        try:
            ...
            energy, fuel_xs = cache.get_cross_section(
                u238, "capture", temperature=600.0
            )
    ...
    def test_get_cross_section_with_self_shielding_subgroup_thermal_energy_group(self):
        ...
        mock_energy = np.array(
            [0.01, 0.1, 1.0, 10.0, 100.0, 500.0, 900.0]
        )  # All < 1 keV
    ...
    def test_get_cross_section_with_self_shielding_subgroup_epithermal_energy_group(
        self,
    ):
...
class TestSelfShieldingIntegrationEdgeCases:
    ...
    def test_get_cross_section_with_self_shielding_subgroup_non_resonance_region_path(
        self,
    ):
        ...
        mock_energy = np.array(
            [0.01, 0.1, 1e6, 1e7]
        )  # No resonance region (1 eV to 100 keV)
        mock_xs = np.array([1.0, 2.0, 3.0, 4.0])
    ...
    def test_get_cross_section_with_self_shielding_cross_section_import_error(self):
        ...
        with patch.object(
            cache, "get_cross_section", side_effect=ImportError("Import failed")
        ):
    ...
    def test_get_cross_section_with_self_shielding_cross_section_value_error(self):
        ...
        with patch.object(
            cache, "get_cross_section", side_effect=ValueError("Invalid data")
        ):
    ...
    def test_subgroup_method_non_resonance_mask_applied(self):
        ...
        with patch.object(
            ...
            with patch(
                ...
                with patch(
                    "smrforge.core.self_shielding_integration.BondarenkoMethod",
                    return_value=mock_bondarenko,
                ):
                    try:
                        energy, xs = get_cross_section_with_self_shielding(
                            cache,
