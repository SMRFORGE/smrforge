            ...
            with patch('smrforge.cli.console') as mock_console:
                with patch('smrforge.gui.run_server', mock_run_server):
                    ...
                    with patch.dict('sys.modules', {
                        'dash': Mock(),
                        'dash_bootstrap_components': Mock()
                    }):
                        cli_module.serve_dashboard(args)
    ...
    def test_serve_dashboard_import_error(self):
        ...
        with patch('smrforge.cli.sys.exit') as mock_exit:
            with patch.dict('sys.modules', {
                ...
                with patch('smrforge.gui', side_effect=ImportError("No module named 'smrforge.gui'")):
                    cli_module.serve_dashboard(args)
                    mock_exit.assert_called_once_with(1)
...
class TestDataSetup:
    ...
    def test_data_setup_interactive(self):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.core.endf_setup.setup_endf_data_interactive', mock_setup):
    ...
    def test_data_setup_directory_not_found(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit') as mock_exit:
            with patch('smrforge.core.endf_setup.setup_endf_data_interactive', side_effect=Exception("Interactive mode failed")):
                cli_module.data_setup(args)
...
class TestDataDownload:
    ...
    def test_data_download_basic(self):
        ...
        mock_download = Mock(return_value={
            'downloaded': 10,
            'skipped': 2,
            'failed': 0,
            'output_dir': '/tmp/endf'
...
class TestDataValidate:
    ...
    def test_data_validate_files(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.core.reactor_core.NuclearDataCache._validate_endf_file', return_value=True):
                with patch('smrforge.cli._RICH_AVAILABLE', False):
                    cli_module.data_validate(args)
...
class TestMainFunction:
    ...
    def test_main_serve_command(self):
        ...
        with patch('sys.argv', ['smrforge', 'serve', '--host', '0.0.0.0', '--port', '9000']):
    ...
    def test_main_reactor_create_command(self):
        ...
        with patch('sys.argv', ['smrforge', 'reactor', 'create', '--preset', 'valar-10']):
...
""")
        ...
        mock_reactor = Mock()
        mock_reactor.spec = Mock(name='reactor', power_thermal=100e6, enrichment=0.05, reactor_type='htgr')
        mock_create_reactor = Mock(return_value=mock_reactor)
...
class TestTransientRun:
    ...
    def test_transient_run_basic(self):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.convenience.transients.quick_transient', mock_quick_transient):
...
class TestThermalLumped:
    ...
    def test_thermal_lumped_basic(self):
        ...
        mock_solver = Mock()
        mock_result = Mock()
        mock_result.to_dict.return_value = {'time': [0, 3600], 'temperature': [500, 600]}
...
class TestValidateDesign:
    ...
    def test_validate_design_with_preset(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.convenience.create_reactor', mock_create_reactor, create=True):
                with patch('smrforge.validation.constraints.DesignValidator', return_value=mock_validator):
                    with patch('smrforge.validation.constraints.ConstraintSet.get_regulatory_limits', return_value=Mock()):
                        with patch('smrforge.cli._print_success') as mock_success:
                            cli_module.validate_design(args)
                            assert mock_success.called
...
class TestReactorAnalyzeBatch:
        ...
        with patch('smrforge.cli.glob.glob', return_value=[str(reactor_file1)]):
            with patch('smrforge.cli.sys.exit'):
                with patch('smrforge.cli._RICH_AVAILABLE', False):
                    ...
                    with patch('smrforge.convenience.create_reactor', side_effect=Exception("Test error"), create=True):
                        cli_module._reactor_analyze_batch(args)
    ...
    def test_reactor_analyze_batch_summary_with_errors_rich(self, tmp_path):
        ...
        with patch('smrforge.cli.glob.glob', return_value=[str(reactor_file1), str(reactor_file2)]):
            with patch('smrforge.cli.sys.exit'):
                with patch('smrforge.convenience.create_reactor', side_effect=create_reactor_side_effect, create=True):
                    with patch('smrforge.cli._RICH_AVAILABLE', True):
                        with patch('smrforge.cli.console') as mock_console:
                            cli_module._reactor_analyze_batch(args)
...
class TestValidateRun:
    ...
    def test_validate_run_script_not_found(self):
        ...
        with patch('pathlib.Path') as mock_path:
            mock_path.return_value.parent.parent.__truediv__ = lambda self, other: mock_script_path
...
class TestVisualizeGeometry:
    ...
    def test_visualize_geometry_2d(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.convenience.create_reactor', mock_create_reactor, create=True):
                with patch('smrforge.visualization.geometry.plot_core_layout', mock_plot):
                    cli_module.visualize_geometry(args)
    ...
    def test_visualize_geometry_3d(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.convenience.create_reactor', mock_create_reactor, create=True):
                with patch('smrforge.visualization.advanced.plot_ray_traced_geometry', mock_plot):
                    cli_module.visualize_geometry(args)
                    assert mock_plot.called
    ...
    def test_visualize_geometry_with_output(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.convenience.create_reactor', mock_create_reactor, create=True):
                with patch('smrforge.visualization.geometry.plot_core_layout', mock_plot):
                    with patch('smrforge.visualization.advanced.export_visualization', mock_export):
                        cli_module.visualize_geometry(args)
...
class TestTemplateCreate:
    ...
    def test_template_create_from_preset(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit'):
            with patch('smrforge.workflows.templates.ReactorTemplate.from_preset', mock_from_preset):
                cli_module.template_create(args)
...
class TestTemplateModify:
    ...
    def test_template_modify_file_not_found(self, tmp_path):
        ...
        with patch('smrforge.cli.sys.exit') as mock_exit:
            with patch('smrforge.workflows.templates.ReactorTemplate.load', side_effect=FileNotFoundError):
