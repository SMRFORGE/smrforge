...
class TestGenerateMultigroup:
        ...
        with patch.object(pre_populated_cache, "get_cross_section") as mock_get:
            ...
            with patch.object(
                ...
                assert np.array_equal(
                    call_args[0][2], group_structure
                )  # group_structure
                assert call_args[0][3] is None  # weighting_flux (None by default)

    def test_generate_multigroup_multiple_nuclides_reactions(
    ...
    def test_generate_multigroup_different_temperatures(
        self, temp_dir, pre_populated_cache
    ):
        ...
        with patch.object(pre_populated_cache, "get_cross_section") as mock_get:
            ...
            with patch.object(
                ...
                for temp in temperatures:
                    ...
                    calls = [
                        call[0][2] for call in mock_get.call_args_list
                    ]  # Extract temperature arg
                    assert temp in calls

    def test_generate_multigroup_group_structure_validation(
        self, temp_dir, pre_populated_cache
    ):
        ...
        with patch.object(pre_populated_cache, "get_cross_section") as mock_get:
            ...
            for group_structure, expected_n_groups in test_cases:
                with patch.object(
                    CrossSectionTable,
                    "_collapse_to_multigroup",
                    return_value=np.ones(expected_n_groups) * 10.0,
                ) as mock_collapse:
                    df = table.generate_multigroup(
                        nuclides=[u235],
                        reactions=["total"],
