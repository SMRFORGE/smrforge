...
class TestParametricVariations:
    ...
    @pytest.mark.parametrize("core_diameter,core_height", [
        (100.0, 200.0),
        (200.0, 400.0),
        (300.0, 600.0),
    ])
    def test_different_core_sizes(self, simple_xs_data, solver_options, core_diameter, core_height):
    ...
    @pytest.mark.parametrize("n_radial,n_axial", [
        (6, 11),
        (11, 21),
        (21, 41),
    ])
    def test_different_mesh_sizes(self, simple_xs_data, solver_options, n_radial, n_axial):
...
class TestEdgeCases:
    ...
    def test_subcritical_configuration(self, simple_geometry, subcritical_xs_data, solver_options):
        """Test solver with subcritical configuration."""
        solver = MultiGroupDiffusion(simple_geometry, subcritical_xs_data, solver_options)
        k_eff, flux = solver.solve_steady_state()
        
        assert k_eff < 1.0, "Should be subcritical"
        assert_solution_reasonable(k_eff, flux, k_eff_range=(0.3, 1.0))
    
    def test_supercritical_configuration(self, simple_geometry, supercritical_xs_data, solver_options):
        """Test solver with supercritical configuration."""
        solver = MultiGroupDiffusion(simple_geometry, supercritical_xs_data, solver_options)
        k_eff, flux = solver.solve_steady_state()
    ...
    def test_high_scattering_ratio(self, simple_geometry, solver_options):
        ...
        sigma_s = np.array([
            [[0.999, 0.001], [0.0, 1.999]]  # High scattering
        ])
...
class TestSolutionQuality:
    ...
    def test_group_flux_ratios(self, solved_solver):
        ...
        assert 0.3 <= fast_ratio <= 0.9, \
            f"Unreasonable fast flux ratio: {fast_ratio}"
