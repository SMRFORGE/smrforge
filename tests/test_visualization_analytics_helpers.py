...
def test_sweep_plots_plotly_smoke():
    ...
    results = [
        {"parameters": {"enrichment": 0.15, "power_mw": 8.0}, "k_eff": 0.99, "success": True},
        {"parameters": {"enrichment": 0.15, "power_mw": 10.0}, "k_eff": 1.00, "success": True},
        {"parameters": {"enrichment": 0.195, "power_mw": 8.0}, "k_eff": 1.01, "success": True},
        {"parameters": {"enrichment": 0.195, "power_mw": 10.0}, "k_eff": 1.02, "success": True},
    ]

    _assert_plotly_fig(
        plot_sweep_heatmap(results, x_param="enrichment", y_param="power_mw", metric="k_eff", backend="plotly")
    )
    ...
    _assert_plotly_fig(plot_sweep_pareto(results, metric_x="k_eff", metric_y="power_mw", backend="plotly"))


@pytest.mark.skipif(not _has_matplotlib(), reason="matplotlib not installed")
def test_sweep_plots_matplotlib_smoke():
    from smrforge.visualization.sweep_plots import plot_sweep_heatmap, plot_sweep_tornado
    ...
    _assert_mpl_fig(
        plot_sweep_heatmap(results, x_param="enrichment", y_param="power_mw", metric="k_eff", backend="matplotlib")
    )
...
def test_uq_visualization_plotly_smoke():
    ...
    from smrforge.uncertainty.visualization import plot_uq_correlation_matrix, plot_uq_distribution
...
def test_neutronics_dashboard_plotly_smoke():
    from smrforge.visualization.tally_data import plot_flux_spectrum_comparison, plot_neutronics_dashboard

    ng = 10
    energy_groups = np.logspace(6, -3, ng + 1)
    flux_a = np.ones((5, 4, ng))
    flux_b = 2.0 * np.ones((5, 4, ng))
    _assert_plotly_fig(plot_flux_spectrum_comparison({"a": flux_a, "b": flux_b}, energy_groups, backend="plotly"))
    _assert_plotly_fig(plot_neutronics_dashboard(flux_a, energy_groups, k_eff=1.0, backend="plotly"))
...
def test_burnup_plots_plotly_smoke():
    ...
    inv = NuclideInventory(nuclides=nuclides, concentrations=conc, times=t_s, burnup=burnup)
...
def test_mesh_validation_econ_opt_plotly_smoke():
    ...
    q = MeshQuality(min_angle=30.0, max_angle=120.0, aspect_ratio=2.0, skewness=0.8, jacobian=1.0)
    sizes = np.random.default_rng(0).lognormal(size=200)
    _assert_plotly_fig(plot_mesh_verification_dashboard(quality=q, sizes=sizes, backend="plotly"))

    vr = ValidationResult(valid=True)
    vr.add_issue(ValidationLevel.ERROR, "pressure", "Negative", value=-1.0, expected=">=0")
    _assert_plotly_fig(plot_validation_summary(vr, backend="plotly"))

    cap = {"direct_reactor_island": 1.0e9, "contingency": 2.0e8, "total_overnight_cost": 1.2e9}
...
def test_sweep_plots_plotly_smoke():
    ...
    _assert_plotly_fig(plot_sweep_heatmap(results, x_param="enrichment", y_param="power_mw", metric="k_eff", backend="plotly"))
    _assert_plotly_fig(plot_sweep_tornado(results, metric="k_eff", backend="plotly"))
    _assert_plotly_fig(plot_sweep_correlation_matrix(results, backend="plotly"))
    _assert_plotly_fig(plot_sweep_pareto(results, metric_x="k_eff", metric_y="power_mw", backend="plotly"))
...
def test_sweep_plots_matplotlib_smoke():
    ...
    _assert_mpl_fig(plot_sweep_heatmap(results, x_param="enrichment", y_param="power_mw", metric="k_eff", backend="matplotlib"))
    _assert_mpl_fig(plot_sweep_tornado(results, metric="k_eff", backend="matplotlib"))
...
def test_mesh_validation_econ_opt_plotly_smoke():
    ...
    cap = {"direct_reactor_island": 1.0e9, "contingency": 2.0e8, "total_overnight_cost": 1.2e9}
    _assert_plotly_fig(plot_capex_breakdown(cap, backend="plotly", kind="waterfall"))
    lcoe = {"capital_contribution": 0.04, "operating_contribution": 0.02, "decommissioning_contribution": 0.005, "total_lcoe": 0.065}
    _assert_plotly_fig(plot_lcoe_breakdown(lcoe, backend="plotly"))

    hist = [{"generation": 0, "best_fitness": 1.0}, {"generation": 1, "best_fitness": 0.7}, {"generation": 2, "best_fitness": 0.55}]
    _assert_plotly_fig(plot_optimization_trace(hist, backend="plotly"))
