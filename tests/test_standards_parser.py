...
class TestStandardsParserExtended:
    ...
    def test_parse_iaea_benchmark_stem_fallback(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="iaea_test_") as f:
            json.dump(test_data, f)
            temp_file = Path(f.name)
    ...
    def test_parse_custom_benchmark(self):
        ...
        try:
            benchmark = parser.parse_custom_benchmark(temp_file, standard_name="CUSTOM_STANDARD")
    ...
    def test_parse_custom_benchmark_stem_fallback(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="custom_") as f:
            json.dump(test_data, f)
            temp_file = Path(f.name)
    ...
    def test_load_into_database_exception_handling(self):
        ...
        benchmark = StandardsBenchmarkData(
            ...
            data={"invalid": "data"},  # Missing required fields like nuclides, initial_power
            metadata={},
        )
        
        # Mock add_decay_heat_benchmark to raise exception
        with patch.object(database, 'add_decay_heat_benchmark', side_effect=ValueError("Invalid data")):
            loaded_count = parser.load_into_database([benchmark], database)
    ...
    def test_parse_standards_data_auto_detect_ansi(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="ansi_") as f:
            json.dump(test_data, f)
            temp_file = Path(f.name)
    ...
    def test_parse_standards_data_auto_detect_iaea(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="iaea_") as f:
            json.dump(test_data, f)
            temp_file = Path(f.name)
    ...
    def test_parse_standards_data_auto_detect_mcnp(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, prefix="mcnp_") as f:
            json.dump(test_data, f)
            temp_file = Path(f.name)
    ...
    def test_parse_standards_data_specified_type_ansi(self):
        ...
        try:
            benchmarks = parse_standards_data(temp_file, standard_type=StandardType.ANSI_ANS_5_1)
            assert len(benchmarks) > 0
            assert benchmarks[0].standard_type == StandardType.ANSI_ANS_5_1
        finally:
            temp_file.unlink()
    ...
    def test_parse_standards_data_specified_type_iaea(self):
        ...
        try:
            benchmarks = parse_standards_data(temp_file, standard_type=StandardType.IAEA_BENCHMARK)
            assert len(benchmarks) > 0
            assert benchmarks[0].standard_type == StandardType.IAEA_BENCHMARK
        finally:
            temp_file.unlink()
    ...
    def test_parse_standards_data_specified_type_mcnp(self):
        ...
        try:
            benchmarks = parse_standards_data(temp_file, standard_type=StandardType.MCNP_REFERENCE)
            assert len(benchmarks) > 0
            assert benchmarks[0].standard_type == StandardType.MCNP_REFERENCE
        finally:
            temp_file.unlink()
    ...
    def test_parse_standards_data_specified_type_custom(self):
        ...
        try:
            benchmarks = parse_standards_data(temp_file, standard_type=StandardType.CUSTOM)
            assert len(benchmarks) > 0
            assert benchmarks[0].standard_type == StandardType.CUSTOM
        finally:
            temp_file.unlink()
