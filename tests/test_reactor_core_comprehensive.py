...
class TestNuclearDataCacheComprehensive:
    ...
    def test_validate_endf_file(self, mock_endf_file, tmp_path):
        ...
        endf_content = (
            "  -1" + " " * 60 + "\n" + (" " * 80 + "\n") * 1000
        )  # ENDF format marker + enough content
    ...
    def test_add_file_to_index_with_duplicate(self, temp_cache_dir, mock_endf_file):
        ...
        cache = NuclearDataCache(
            cache_dir=temp_cache_dir, local_endf_dir=mock_endf_file.parent
        )
        ...
        nuclide1 = cache._add_file_to_index(
            test_file1, index, allow_alternative_names=False
        )
        ...
        nuclide2 = cache._add_file_to_index(
            test_file2, index, allow_alternative_names=True
        )
    ...
    def test_add_file_to_index_with_filename_mismatch(self, temp_cache_dir, tmp_path):
        ...
        valid_endf.write_text(
            "  -1" + " " * 60 + "\n" + (" " * 80 + "\n") * 1000, encoding="utf-8"
        )

        index = {}
        nuclide = cache._add_file_to_index(
            valid_endf, index, allow_alternative_names=False
        )
    ...
    def test_add_file_to_index_nonexistent_file(self, temp_cache_dir):
        ...
        nuclide = cache._add_file_to_index(
            nonexistent_file, index, allow_alternative_names=False
        )
        assert nuclide is None
        assert len(index) == 0

    def test_build_local_file_index_with_alternative_naming(
