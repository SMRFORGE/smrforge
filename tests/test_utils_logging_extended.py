...
class TestLogSolverIterationExtended:
    ...
    def test_log_solver_iteration_format_validation(self):
        ...
        logging_module.log_solver_iteration(
            42, 1.12345678, 1.234e-7, logger=mock_logger
        )
        ...
        assert (
            "1.23e-07" in call_args
            or "1.234e-07" in call_args
            or "1.23e-7" in call_args
        )
...
class TestLogCacheOperationExtended:
    ...
    def test_log_cache_operation_all_operations(self):
        ...
        for operation in operations:
            logging_module.log_cache_operation(
                operation, "test_key", logger=mock_logger
            )
            call_args = mock_logger.debug.call_args[0][0]
...
class TestIntegrationScenarios:
    ...
    def test_full_logging_workflow(self, tmp_path):
        ...
        logging_module.log_nuclear_data_fetch(
            "U235", "fission", 1200.0, "sandy", logger=logger
        )
