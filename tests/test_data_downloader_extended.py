...
class TestDownloadEndfDataExtended:
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_unknown_library_string(
        self, mock_validate, mock_download
    ):
    ...
    def test_download_endf_data_resume_with_validation(
        self, mock_exists, mock_download, mock_validate
    ):
    ...
    def test_download_endf_data_resume_invalid_file(
        self, mock_unlink, mock_exists, mock_download, mock_validate
    ):
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.organize_bulk_endf_downloads")
    def test_download_endf_data_organize_no_downloads(
        self, mock_organize, mock_download
    ):
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_sequential_with_validation(
        self, mock_validate, mock_download
    ):
    ...
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    @patch("pathlib.Path.unlink")
    def test_download_endf_data_sequential_invalid_then_valid(
        self, mock_unlink, mock_validate, mock_download
    ):
...
class TestDownloadParallelExtended:
    ...
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    @patch("pathlib.Path.unlink")
    def test_download_parallel_validation_failure(
        self, mock_unlink, mock_validate, mock_download
    ):
        ...
        download_tasks = [
            (
                nuclides[0],
                Path("test.endf"),
                ["http://url1.com/file.endf", "http://url2.com/file.endf"],
            )
        ]
        stats = {"downloaded": 0, "failed": 0}
    ...
    def test_download_parallel_caches_successful_source(
        self, mock_cache, mock_download
    ):
        ...
        download_tasks = [(nuclides[0], Path("test.endf"), [test_url])]
        stats = {"downloaded": 0, "failed": 0}
...
class TestDownloadPreprocessedLibraryExtended:
    ...
    @patch("smrforge.data_downloader.download_endf_data")
    @patch("smrforge.data_downloader.REQUESTS_AVAILABLE", True)
    def test_download_preprocessed_library_with_nuclide_set_common_smr(
        self, mock_download
    ):
        """Test download_preprocessed_library with nuclides='common_smr' calls download_endf_data with nuclide_set."""
        if not DATA_DOWNLOADER_AVAILABLE:
            pytest.skip("Data downloader module not available")
        mock_download.return_value = {
            "downloaded": 10,
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_download.assert_called_once()
            call_kwargs = mock_download.call_args[1]
            assert call_kwargs.get("nuclide_set") == "common_smr"
            assert "nuclides" not in call_kwargs or call_kwargs.get("nuclides") is None
...
class TestDownloadEndfDataCoverage90:
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_default_uses_common_smr(
        self, mock_validate, mock_download
    ):
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_nuclide_set_common_smr_explicit(
        self, mock_validate, mock_download
    ):
        """Explicit nuclide_set='common_smr' uses COMMON_SMR_NUCLIDES (branch distinct from default)."""
        if not DATA_DOWNLOADER_AVAILABLE:
            pytest.skip("Data downloader module not available")
        mock_download.return_value = True
        mock_validate.return_value = True
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result["total"] > 0
            assert "downloaded" in result
            assert result["total"] <= len(downloader_module.COMMON_SMR_NUCLIDES)
    ...
    def test_expand_elements_to_nuclides_unknown_element_skipped(self):
        ...
        nuclides_mixed = downloader_module._expand_elements_to_nuclides(
            ["Xx", "U"], Library.ENDF_B_VIII_1
        )
    ...
    def test_download_endf_data_organize_called_after_downloads(self, mock_download):
        ...
        mock_download.return_value = True
        with patch(
            "smrforge.data_downloader.organize_bulk_endf_downloads"
        ) as mock_organize:
    ...
    @patch("smrforge.data_downloader.REQUESTS_AVAILABLE", True)
    @patch("smrforge.data_downloader.download_file")
    def test_download_endf_data_organize_uses_viii0_for_endf_b_viii(
        self, mock_download
    ):
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_output_dir_string(self, mock_validate, mock_download):
        """output_dir as str (not Path) is accepted and resolved."""
        if not DATA_DOWNLOADER_AVAILABLE:
            pytest.skip("Data downloader module not available")
        mock_download.return_value = True
        mock_validate.return_value = True
    ...
    @patch("smrforge.data_downloader.REQUESTS_AVAILABLE", True)
    @patch("smrforge.data_downloader.download_file")
    def test_download_endf_data_library_string_viii0_maps_to_enum(self, mock_download):
        """library='ENDF/B-VIII.0' string maps to Library.ENDF_B_VIII and organize uses VIII.0."""
        if not DATA_DOWNLOADER_AVAILABLE:
            pytest.skip("Data downloader module not available")
        mock_download.return_value = True
        with patch(
    ...
    @patch("smrforge.data_downloader.download_file")
    @patch("smrforge.data_downloader.NuclearDataCache._validate_endf_file")
    def test_download_endf_data_tqdm_unavailable_completes(
        self, mock_validate, mock_download
    ):
    ...
    @patch("smrforge.data_downloader.REQUESTS_AVAILABLE", True)
    @patch("smrforge.data_downloader.download_endf_data")
    def test_download_preprocessed_library_nuclides_list_calls_download_endf_data(
        self, mock_download
    ):
...
class TestDownloadEndfDataCoverage90:
    ...
    def test_get_endf_url_isomer_includes_meta_suffix(self):
        ...
        assert "m1" in url
        assert (
            url.endswith("n-092_U_239m1.endf") or "239m1" in url or "_239m1.endf" in url
        )
...
class TestHelperFunctionsAdditional:
    ...
    def test_cache_successful_source_edge_cases(self):
        ...
        downloader_module._cache_successful_source(
            "https://example.com/file.endf", Library.ENDF_B_VIII_1
        )
        ...
        downloader_module._cache_successful_source(
            "https://www.nndc.bnl.gov/other/path/file.endf", Library.ENDF_B_VIII_1
        )
        # Should cache as nndc
        assert (
            downloader_module._source_cache.get(Library.ENDF_B_VIII_1.value) == "nndc"
        )
        ...
        assert (
            downloader_module._source_cache.get(Library.ENDF_B_VIII_1.value) == "iaea"
        )
