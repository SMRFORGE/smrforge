...
class ValidationBenchmarker:
    ...
    def __init__(
        self, cache: NuclearDataCache, benchmark_database: Optional[Any] = None
    ):
        """
    ...
    def time_function(self, func, *args, iterations: int = 1, **kwargs) -> TimingResult:
        ...
        return TimingResult(
            name=func.__name__, elapsed_time=elapsed, iterations=iterations
        )
    ...
    def validate_tsl_interpolation_accuracy(
        ...
        try:
            ...
            tsl_data_list = [
                (temperatures[0] if len(temperatures) > 0 else 293.6, tsl_data)
            ]
    ...
    def validate_fission_yield_parser(self, nuclide: Nuclide) -> ValidationResult:
        ...
        try:
            ...
            total_yield = (
                yield_data.get_total_yield()
                if hasattr(yield_data, "get_total_yield")
                else 0.0
            )

            # Count independent and cumulative yields
            n_independent = sum(
                1 for fy in yield_data.yields.values() if fy.independent_yield > 0
            )
            n_cumulative = sum(
                1 for fy in yield_data.yields.values() if fy.cumulative_yield > 0
            )
    ...
    def validate_decay_heat_ans_standard(
        ...
        try:
            ...
            timing = self.time_function(
                calc.calculate_decay_heat, concentrations=nuclides, times=times
            )
            ...
            if len(result.total_decay_heat) > 1:
                ...
                mostly_decreasing = (
                    np.mean(decay_rate[1:] < 0) > 0.5
                )  # More than half should decrease
            else:
            ...
            metrics = {
                ...
                "decay_heat_range": (
                    float(np.min(result.total_decay_heat)),
                    float(np.max(result.total_decay_heat)),
                ),
                "final_decay_heat": (
                    float(result.total_decay_heat[-1])
                    if len(result.total_decay_heat) > 0
                    else 0.0
                ),
            }
            ...
            return ValidationResult(
                ...
                passed=mostly_decreasing
                and np.all(np.isfinite(result.total_decay_heat)),
    ...
    def validate_cross_section_spot_check(
        ...
        try:
            ...
            comparison = compare_with_benchmark(
                calculated_value, benchmark_value, tolerance=tolerance
            )
    ...
    def benchmark_k_eff(
        ...
        try:
            ...
            for i, (calc_k, bench_k) in enumerate(
                zip(calculated_k_eff, benchmark.benchmark_k_eff)
            ):
                comparison = compare_with_benchmark(
                    calc_k, bench_k, tolerance=tolerance
                )
            ...
            metrics = {
                ...
                "within_tolerance_count": sum(
                    1 for c in comparisons if c["within_tolerance"]
                ),
