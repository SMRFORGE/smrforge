...
def create_mock_endf_file(
    ...
    header = f" 9.{nuclide.Z:02d}{nuclide.A:03d}+4 2.345678+2          0          0          0          0  0  0     \n"
    lines.append(header)
    ...
    mf3_control = f" 0.000000+0 0.000000+0          0          0          1         {n_points:4d}  3  {reaction_mt:3d}     \n"
    lines.append(mf3_control)

    # Interpolation law (linear interpolation, law=2)
    mf3_interp = f" 0.000000+0 0.000000+0          0          0          0          0  3  {reaction_mt:3d}     \n"
    ...
    if include_other_reactions:
        ...
        for mt, _, e_vals, xs_vals in other_reactions:
            ...
            # Section header
            mf3_header_other = f" 9.{nuclide.Z:02d}{nuclide.A:03d}+4 2.345678+2          0          0          0          0  3  {mt:3d}     \n"
            lines.append(mf3_header_other)

            # Control record
            mf3_control_other = f" 0.000000+0 0.000000+0          0          0          1         {len(e_vals):4d}  3  {mt:3d}     \n"
            lines.append(mf3_control_other)

            # Interpolation law
            mf3_interp_other = f" 0.000000+0 0.000000+0          0          0          0          0  3  {mt:3d}     \n"
