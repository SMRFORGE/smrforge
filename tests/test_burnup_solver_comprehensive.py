...
def mock_cache():
    ...
    cache.get_cross_section = Mock(
        return_value=(np.array([1e6, 2e6]), np.array([1.0, 2.0]))
    )
    return cache
...
class TestBurnupSolverComprehensive:
    ...
    def test_build_fission_production_vector(self, simple_neutronics, mock_cache):
        ...
        burnup.neutronics.flux = np.ones(
            (burnup.neutronics.nz, burnup.neutronics.nr, n_groups)
        )
    ...
    def test_solve_full_burnup(self, simple_neutronics, mock_cache):
        ...
        with patch.object(
            burnup.neutronics,
            "solve_steady_state",
            return_value=(1.0, np.ones((3, 5, 2))),
        ):
    ...
    def test_power_density_array(self, simple_neutronics, mock_cache):
        ...
        assert len(burnup.options.power_density) == len(
            options.time_steps
        ) or isinstance(burnup.options.power_density, (int, float))
    ...
    def test_initial_concentrations_calculation(self, simple_neutronics, mock_cache):
        ...
        if u235 in burnup.nuclides and u238 in burnup.nuclides:
            ...
            assert (
                burnup.concentrations[idx_u235, 0] < burnup.concentrations[idx_u238, 0]
            )
    ...
    def test_build_capture_matrix_with_cross_section(
        self, simple_neutronics, mock_cache
    ):
    ...
    def test_solve_time_step_exception(self, simple_neutronics, mock_cache):
        ...
        with patch(
            "smrforge.burnup.solver.solve_ivp", side_effect=Exception("Solver error")
        ):
            # Should fall back to Euler step
            burnup._solve_time_step(1, 0.0, 30.0 * 24 * 3600, 30.0 * 24 * 3600)
    ...
    def test_solve_neutronics_error_handling(self, simple_neutronics, mock_cache):
        ...
        with patch.object(
            burnup.neutronics, "solve_steady_state", side_effect=mock_solve
        ):
    ...
    def test_build_fission_production_vector_no_flux(
        self, simple_neutronics, mock_cache
    ):
    ...
    def test_build_capture_matrix_nuclide_specific_xs_failure(
        self, simple_neutronics, mock_cache
    ):
    ...
    def test_build_capture_matrix_zero_concentration(
        self, simple_neutronics, mock_cache
    ):
    ...
    def test_solve_with_neutronics_resolve_condition(
        self, simple_neutronics, mock_cache
    ):
