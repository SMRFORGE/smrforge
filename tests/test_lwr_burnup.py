...
class TestLWRBurnupEdgeCases:
    ...
    def test_gadolinium_depletion_get_capture_cross_section_thermal_energy_not_exact(
        self,
    ):
        ...
        with patch.object(
            gd.cache, "get_cross_section", return_value=(mock_energy, mock_xs)
        ):
            xs = gd.get_capture_cross_section(gd155, temperature=600.0)
    ...
    def test_rod_wise_burnup_tracker_calculate_shadowing_factor_distance_calculation(
        self,
    ):
    ...
    def test_gadolinium_depletion_calculate_reactivity_worth_partial_depletion(self):
        ...
        worth_short = gd.calculate_reactivity_worth(
            poison, 1e14, 365 * 24 * 3600
        )  # 1 year

        # Long time (more depletion)
        worth_long = gd.calculate_reactivity_worth(
            poison, 1e14, 10 * 365 * 24 * 3600
        )  # 10 years
    ...
    def test_gadolinium_depletion_get_capture_cross_section_exception_handling(self):
        ...
        gd155 = Nuclide(Z=64, A=155)
        with patch.object(
            gd.cache, "get_cross_section", side_effect=Exception("Data not available")
        ):
            xs = gd.get_capture_cross_section(gd155)
    ...
    def test_gadolinium_depletion_deplete_exponential_decay_edge_cases(self):
        ...
        with patch.object(
            gd, "get_capture_cross_section", return_value=1e6
        ):  # Very high XS
            final = gd.deplete(
                nuclide=gd155,
                initial_concentration=1e20,
        ...
        with patch.object(
            gd, "get_capture_cross_section", return_value=1.0
        ):  # Very low XS
            final = gd.deplete(
                nuclide=gd155,
                initial_concentration=1e20,
