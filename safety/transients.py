...
if __name__ == "__main__":
    ...
    result_atws = atws.simulate(atws_conditions)

    # Test 3: Reactivity Insertion Accident
    console.print("\n[bold]Test 3: Reactivity Insertion Accident[/bold]")
    ria = ReactivityInsertionAccident(reactor.spec)
    result_ria = ria.simulate(
        rho_inserted=0.004,  # $0.60 (60 cents)
        insertion_time=0.1,  # 100 ms
        conditions=base_conditions,
    )

    # Test 4: Air Ingress
    console.print("\n[bold]Test 4: Air Ingress[/bold]")
    air_ingress = AirWaterIngressAnalysis(reactor.spec, core)
    result_air = air_ingress.simulate_air_ingress(
        break_size=100.0,  # 100 cm² break
        conditions=base_conditions,
    )

    console.print("\n[bold green]All transient scenarios completed![/bold green]")
