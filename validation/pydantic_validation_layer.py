...
if __name__ == "__main__":
    ...
    try:
        ...
        console.print("  ✓ Cross sections validated")
        console.print(f"    Shape: {xs.sigma_t.shape}")
    except Exception as e:
        console.print(f"  ✗ Error: {e}")

    # Test 6: Settings from environment
    console.print("\n[bold]Test 6: Settings[/bold]")
    settings = SMRForgeSettings()
    console.print(f"  Cache dir: {settings.cache_dir}")
    console.print(f"  Nuclear data library: {settings.nuclear_data_library}")
    console.print(f"  Threads: {settings.n_threads}")

    console.print("\n[bold green]Pydantic validation layer ready![/bold green]")
