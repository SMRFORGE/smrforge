...
2. **Add tests** for modules with the most uncovered lines (see "Modules Below 75% Target" below). Highest impact: `geometry/advanced_import.py`, `geometry/validation.py`, `data_downloader.py`, `neutronics/*` (hybrid_solver, adaptive_sampling, monte_carlo_optimized, implicit_mc), `core/multigroup_advanced.py`, `core/endf_setup.py`, `burnup/lwr_burnup.py`, `convenience.py`. **Recent additions (Jan 2026):** `test_data_downloader_extended.TestDownloadEndfDataCoverage90`: download_endf_data when REQUESTS_AVAILABLE=False (ImportError); default common_smr path; **_get_endf_url isomer (m>0) includes meta suffix** (test_get_endf_url_isomer_includes_meta_suffix); **_get_download_urls default IAEA first** (test_get_download_urls_default_iaea_first); **_get_download_urls prefers NNDC when cached** (test_get_download_urls_prefers_nndc_when_cached); **_cache_successful_source iaea.org branch** (in test_cache_successful_source_edge_cases); _expand_elements_to_nuclides unknown element skipped + mixed invalid/valid; organize called after downloads; organize uses library_version VIII.0 for ENDF_B_VIII; download_preprocessed_library with nuclides list calls download_endf_data(nuclides=...); test_download_endf_data_tqdm_unavailable_completes (TQDM_AVAILABLE=False, show_progress=True path). `test_lwr_burnup.TestGadoliniumDepletion`: test_deplete_negative_flux_returns_initial, test_deplete_negative_time_returns_initial (flux ≤ 0 / time ≤ 0 paths). `test_control_integration_extended`: test_create_controlled_reactivity_state_with_t_fuel, test_create_load_following_reactivity_state_with_t_fuel (T_fuel state branch); **test_create_controlled_reactivity_with_rod_worth_passed** (rod_worth passed explicitly, covers rod_worth is not None branch). `test_decay_chain_utils`: build_fission_product_chain max_depth=0. `test_geometry_validation` / `test_geometry_validation_extended`: validate_material_connectivity check_continuity/check_boundaries=False; check_distances_and_clearances ImportError when _GEOMETRY_TYPES_AVAILABLE=False; **test_check_gaps_severity_warning_large_gap** (check_gaps_and_boundaries severity "warning" when gap_size > max_gap + tolerance); **test_comprehensive_validation_clearances_only** (comprehensive_validation with only check_clearances=True); **test_comprehensive_validation_connectivity_only**; **test_comprehensive_validation_assemblies_only**; **test_comprehensive_validation_control_rods_only**; **test_validate_prismatic_block_invalid_height** (validate_geometry_completeness when block has height ≤ 0). `test_multigroup_advanced`: zero_denominator_fallback uses isfinite assertion.
